#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano adopt-nestjs-config` em 2026-09-05 00:18
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo adopt-nestjs-config
set -u
set -o pipefail

RUN_ID='desafio-dev-backend-senior-adopt-nestjs-config-mtnmx7pb'
FEATURE='adopt-nestjs-config'
BASE_BRANCH='spec/adopt-nestjs-config'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/adopt-nestjs-config/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-adopt-nestjs-config-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-adopt-nestjs-config"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/adopt-nestjs-config/executar-tarefas.sh --faixa $1"
...
# ── sequencial T-176 (ordem do tasks.md) ──
executar_seq_T_176() {
  info 'sequencial T-176 — Replace custom environment plumbing with NestJS Config'
  if rodar_tarefa seq 'T-176' 'Você executa UMA tarefa da feature "adopt-nestjs-config" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/adopt-nestjs-config/spec.md, .spec/features/adopt-nestjs-config/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-176 — "Replace custom environment plumbing with NestJS Config"
  critérios/refs: AC-216 (Official configuration is active in every NestJS application), AC-217 (Existing runtime configuration behavior is preserved), AC-218 (Custom environment plumbing is removed)
  arquivos permitidos (e seus testes): package.json, pnpm-lock.yaml, libs/platform/nest/src/config, libs/platform/nest/src/index.ts, libs/gateway/nest/src, libs/identity/nest/src, apps/gateway/src, apps/identity-subgraph/src, apps/order-workflow-subgraph/src, test/nest-provider-composition.test.mjs, test/adopt-nestjs-config.test.mjs, vitest.config.ts, .spec/features/adopt-nestjs-config/evidence/tdd.md
  mensagem de commit: "T-176 adopt-nestjs-config: Replace custom environment plumbing with NestJS Config"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high >> "$LOG_DIR/seq.log" 2>&1; then
    ...
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-176 adopt-nestjs-config: Replace custom environment plumbing with NestJS Config (auto-commit do plano)'
    fi
    marcar_concluidas T-176
    verde "✔ T-176 concluída"
    return 0
  fi
  vermelho "✘ T-176 falhou (log: $LOG_DIR/seq.log)"
  amarelo "  reexecute só ela: bash .spec/features/adopt-nestjs-config/executar-tarefas.sh --seq T-176"
  FALHAS="$FALHAS T-176"
  return 1
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/adopt-nestjs-config/executar-tarefas.sh --gate"
      exit 0
    fi
...
executar_tudo() {
  ...
  executar_seq_T_176 || true
  encerrar tudo
}

listar() {
  echo "execução: $RUN_ID (feature $FEATURE, branch $BASE_BRANCH)"
  echo "  seq       T-176 (sequencial)"
  echo
...
case "$MODO" in
  ...
  seq)
    case "$ALVO" in
      T-176) evento --tipo inicio --escopo "seq:T-176"; iniciar_resumos; executar_seq_T_176 || true; encerrar "seq:T-176" ;;
