...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo align-milestone-7-checkout-contract
set -u
set -o pipefail
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/align-milestone-7-checkout-contract/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-align-milestone-7-checkout-contract-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-align-milestone-7-checkout-contract"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/align-milestone-7-checkout-contract/executar-tarefas.sh --faixa $1"
...
T-303 — "Align Gateway and E2E checkout operation shapes"
  critérios/refs: AC-352 (Gateway checkout composition matches the Java runtime)
  arquivos permitidos (e seus testes): libs/contracts/graphql/order-workflow/schema.graphql, apps/e2e/src/journey.ts, apps/e2e/src/milestone-7.e2e.test.ts, test/milestone-7-e2e-contract.test.mjs
  mensagem de commit: "T-303 align-milestone-7-checkout-contract: Align Gateway and E2E checkout operation shapes"
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/align-milestone-7-checkout-contract/executar-tarefas.sh --gate"
      exit 0
    fi
