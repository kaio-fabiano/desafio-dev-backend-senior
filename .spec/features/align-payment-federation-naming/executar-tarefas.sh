#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano align-payment-federation-naming` em 2026-09-01 23:16
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo align-payment-federation-naming
set -u
set -o pipefail

RUN_ID='desafio-dev-backend-senior-align-payment-federation-naming-mtjady4c'
FEATURE='align-payment-federation-naming'
BASE_BRANCH='spec/align-payment-federation-naming'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/align-payment-federation-naming/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-align-payment-federation-naming-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-align-payment-federation-naming"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/align-payment-federation-naming/executar-tarefas.sh --faixa $1"
...
# ── sequencial T-116 (ordem do tasks.md) ──
executar_seq_T_116() {
  info 'sequencial T-116 — Define executable naming boundaries'
  if rodar_tarefa seq 'T-116' 'Você executa UMA tarefa da feature "align-payment-federation-naming" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/align-payment-federation-naming/spec.md, .spec/features/align-payment-federation-naming/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-116 — "Define executable naming boundaries"
  critérios/refs: AC-152 (Payment Federation has one canonical name), AC-153 (Payment and Inventory remain internal participants), AC-154 (Catalog contract is absent)
  arquivos permitidos (e seus testes): test/align-payment-federation-naming.spec.test.js, test/payment-federation-naming.test.mjs
  mensagem de commit: "T-116 align-payment-federation-naming: Define executable naming boundaries"

Regras inegociáveis:
- Todo critério de aceite referenciado vira teste com @spec:AC-xxx no título.
- NUNCA enfraqueça, pule (skip/todo) ou apague um teste para passar — teste pulado não é prova e o audit acusa.
- Rode os testes localmente com `find test -maxdepth 1 -name '\''*.test.mjs'\'' ! -name '\''marco-0-wordpress.test.mjs'\'' -print0 | xargs -0 env NODE_ENV=test TSX_TSCONFIG_PATH=apps/order-workflow-subgraph/tsconfig.app.json node --import tsx --test --test-reporter=tap` até passarem.
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' medium >> "$LOG_DIR/seq.log" 2>&1; then
    ...
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-116 align-payment-federation-naming: Define executable naming boundaries (auto-commit do plano)'
    fi
    marcar_concluidas T-116
    verde "✔ T-116 concluída"
    return 0
  fi
  vermelho "✘ T-116 falhou (log: $LOG_DIR/seq.log)"
  amarelo "  reexecute só ela: bash .spec/features/align-payment-federation-naming/executar-tarefas.sh --seq T-116"
  FALHAS="$FALHAS T-116"
  return 1
}

# ── sequencial T-117 (ordem do tasks.md) ──
executar_seq_T_117() {
  info 'sequencial T-117 — Rename the Java runtime and active references'
  if rodar_tarefa seq 'T-117' 'Você executa UMA tarefa da feature "align-payment-federation-naming" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/align-payment-federation-naming/spec.md, .spec/features/align-payment-federation-naming/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-117 — "Rename the Java runtime and active references"
  critérios/refs: AC-152 (Payment Federation has one canonical name), AC-153 (Payment and Inventory remain internal participants)
  arquivos permitidos (e seus testes): apps/payment-federation, apps/payment-federation, apps/e2e, compose.yaml, infra/sst.config.ts, libs/contracts/graphql/supergraph.yaml, libs/gateway/nest/src/gateway.module.ts, package.json, test
  mensagem de commit: "T-117 align-payment-federation-naming: Rename the Java runtime and active references"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' medium >> "$LOG_DIR/seq.log" 2>&1; then
    # commit de segurança se o agente esqueceu (rastreabilidade > perfeição)
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-117 align-payment-federation-naming: Rename the Java runtime and active references (auto-commit do plano)'
    fi
    marcar_concluidas T-117
    verde "✔ T-117 concluída"
    return 0
  fi
  vermelho "✘ T-117 falhou (log: $LOG_DIR/seq.log)"
  amarelo "  reexecute só ela: bash .spec/features/align-payment-federation-naming/executar-tarefas.sh --seq T-117"
  FALHAS="$FALHAS T-117"
  return 1
}
...
executar_seq_T_118() {
  info 'sequencial T-118 — Remove the retired contract and align documentation'
  if rodar_tarefa seq 'T-118' 'Você executa UMA tarefa da feature "align-payment-federation-naming" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/align-payment-federation-naming/spec.md, .spec/features/align-payment-federation-naming/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-118 — "Remove the retired contract and align documentation"
  critérios/refs: AC-154 (Catalog contract is absent)
  arquivos permitidos (e seus testes): libs/contracts/graphql/catalog, README.md, docs
  mensagem de commit: "T-118 align-payment-federation-naming: Remove the retired contract and align documentation"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-luna' low >> "$LOG_DIR/seq.log" 2>&1; then
    # commit de segurança se o agente esqueceu (rastreabilidade > perfeição)
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-118 align-payment-federation-naming: Remove the retired contract and align documentation (auto-commit do plano)'
    fi
  ...
  amarelo "  reexecute só ela: bash .spec/features/align-payment-federation-naming/executar-tarefas.sh --seq T-118"
  FALHAS="$FALHAS T-118"
  return 1
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/align-payment-federation-naming/executar-tarefas.sh --gate"
      exit 0
    fi
...
executar_tudo() {
  ...
  executar_seq_T_116 || true
  executar_seq_T_117 || true
...
listar() {
  ...
  echo "  seq       T-116 (sequencial)"
  echo "  seq       T-117 (sequencial)"
...
case "$MODO" in
  ...
  seq)
    case "$ALVO" in
      T-116) evento --tipo inicio --escopo "seq:T-116"; iniciar_resumos; executar_seq_T_116 || true; encerrar "seq:T-116" ;;
      T-117) evento --tipo inicio --escopo "seq:T-117"; iniciar_resumos; executar_seq_T_117 || true; encerrar "seq:T-117" ;;
