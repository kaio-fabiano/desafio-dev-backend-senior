#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano align-wordpress-cart-product-edge` em 2026-09-11 15:12
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo align-wordpress-cart-product-edge
set -u
set -o pipefail

RUN_ID='desafio-dev-backend-senior-align-wordpress-cart-product-edge-mtx3hfck'
FEATURE='align-wordpress-cart-product-edge'
BASE_BRANCH='spec/align-wordpress-cart-product-edge'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/align-wordpress-cart-product-edge/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-align-wordpress-cart-product-edge-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-align-wordpress-cart-product-edge"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/align-wordpress-cart-product-edge/executar-tarefas.sh --faixa $1"
...
# ── sequencial T-307 (ordem do tasks.md) ──
executar_seq_T_307() {
  info 'sequencial T-307 — Align cart product contracts with WooGraphQL'
  if rodar_tarefa seq 'T-307' 'Você executa UMA tarefa da feature "align-wordpress-cart-product-edge" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/align-wordpress-cart-product-edge/spec.md, .spec/features/align-wordpress-cart-product-edge/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-307 — "Align cart product contracts with WooGraphQL"
  critérios/refs: AC-356 (Cart product selections follow the WooGraphQL edge)
  arquivos permitidos (e seus testes): libs/contracts/graphql/wordpress/schema.graphql, apps/apollo-mcp/schema.graphql, apps/apollo-mcp/operations/add-to-cart.graphql, apps/apollo-mcp/operations/get-my-cart.graphql, test/wordpress-native-commerce.test.mjs
  mensagem de commit: "T-307 align-wordpress-cart-product-edge: Align cart product contracts with WooGraphQL"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-luna' low >> "$LOG_DIR/seq.log" 2>&1; then
    ...
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-307 align-wordpress-cart-product-edge: Align cart product contracts with WooGraphQL (auto-commit do plano)'
    fi
    marcar_concluidas T-307
    verde "✔ T-307 concluída"
    return 0
  fi
  vermelho "✘ T-307 falhou (log: $LOG_DIR/seq.log)"
  amarelo "  reexecute só ela: bash .spec/features/align-wordpress-cart-product-edge/executar-tarefas.sh --seq T-307"
  FALHAS="$FALHAS T-307"
  return 1
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/align-wordpress-cart-product-edge/executar-tarefas.sh --gate"
      exit 0
    fi
...
executar_tudo() {
  ...
  executar_seq_T_307 || true
  encerrar tudo
}

listar() {
  echo "execução: $RUN_ID (feature $FEATURE, branch $BASE_BRANCH)"
  echo "  seq       T-307 (sequencial)"
  echo
...
case "$MODO" in
  ...
  seq)
    case "$ALVO" in
      T-307) evento --tipo inicio --escopo "seq:T-307"; iniciar_resumos; executar_seq_T_307 || true; encerrar "seq:T-307" ;;
