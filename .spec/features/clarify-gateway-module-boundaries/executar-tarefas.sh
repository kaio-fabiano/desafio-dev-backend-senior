#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano clarify-gateway-module-boundaries` em 2026-09-05 16:47
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo clarify-gateway-module-boundaries
set -u
set -o pipefail

RUN_ID='desafio-dev-backend-senior-clarify-gateway-module-boundaries-mtom8b21'
FEATURE='clarify-gateway-module-boundaries'
BASE_BRANCH='spec/clarify-gateway-module-boundaries'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/clarify-gateway-module-boundaries/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-clarify-gateway-module-boundaries-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-clarify-gateway-module-boundaries"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/clarify-gateway-module-boundaries/executar-tarefas.sh --faixa $1"
...
# ── sequencial T-202 (ordem do tasks.md) ──
executar_seq_T_202() {
  info 'sequencial T-202 — Extract the gateway authentication module'
  if rodar_tarefa seq 'T-202' 'Você executa UMA tarefa da feature "clarify-gateway-module-boundaries" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/clarify-gateway-module-boundaries/spec.md, .spec/features/clarify-gateway-module-boundaries/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-202 — "Extract the gateway authentication module"
  critérios/refs: AC-245 (Authentication has a focused NestJS module)
  arquivos permitidos (e seus testes): libs/gateway/nest/src/auth/gateway-auth.module.ts, libs/gateway/nest/src/auth/gateway-auth.module.spec.ts, libs/gateway/nest/src/gateway.module.ts, libs/gateway/nest/src/gateway.module.spec.ts, libs/gateway/nest/src/index.ts, test/clarify-gateway-module-boundaries.spec.test.mjs, test/milestone-8-identity-gateway.test.mjs, test/oauth-resource-server-auth.spec.test.mjs, test/structural-mcp-review.test.mjs, docs/evidence/clarify-gateway-module-boundaries/T-202.md
  mensagem de commit: "T-202 clarify-gateway-module-boundaries: Extract the gateway authentication module"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-luna' low >> "$LOG_DIR/seq.log" 2>&1; then
    ...
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-202 clarify-gateway-module-boundaries: Extract the gateway authentication module (auto-commit do plano)'
    fi
    marcar_concluidas T-202
    verde "✔ T-202 concluída"
    return 0
  fi
  vermelho "✘ T-202 falhou (log: $LOG_DIR/seq.log)"
  amarelo "  reexecute só ela: bash .spec/features/clarify-gateway-module-boundaries/executar-tarefas.sh --seq T-202"
  FALHAS="$FALHAS T-202"
  return 1
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/clarify-gateway-module-boundaries/executar-tarefas.sh --gate"
      exit 0
    fi
...
executar_tudo() {
  ...
  executar_seq_T_202 || true
  encerrar tudo
}

listar() {
  echo "execução: $RUN_ID (feature $FEATURE, branch $BASE_BRANCH)"
  echo "  seq       T-202 (sequencial)"
  echo
...
case "$MODO" in
  ...
  seq)
    case "$ALVO" in
      T-202) evento --tipo inicio --escopo "seq:T-202"; iniciar_resumos; executar_seq_T_202 || true; encerrar "seq:T-202" ;;
