#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano connect-graphiql-sse` em 2026-09-11 16:33
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo connect-graphiql-sse
set -u
set -o pipefail

RUN_ID='desafio-dev-backend-senior-connect-graphiql-sse-mtx6ehq9'
FEATURE='connect-graphiql-sse'
BASE_BRANCH='spec/connect-graphiql-sse'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/connect-graphiql-sse/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-connect-graphiql-sse-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-connect-graphiql-sse"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/connect-graphiql-sse/executar-tarefas.sh --faixa $1"
...
# ── sequencial T-309 (ordem do tasks.md) ──
executar_seq_T_309() {
  info 'sequencial T-309 — Serve an SSE-aware GraphiQL page'
  if rodar_tarefa seq 'T-309' 'Você executa UMA tarefa da feature "connect-graphiql-sse" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/connect-graphiql-sse/spec.md, .spec/features/connect-graphiql-sse/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-309 — "Serve an SSE-aware GraphiQL page"
  critérios/refs: AC-359 (Subscription usa o endpoint SSE do Gateway)
  arquivos permitidos (e seus testes): apps/gateway/src/graphiql/gateway-graphiql-page.spec.ts, apps/gateway/src/graphiql/gateway-graphiql-page.ts, apps/gateway/src/main.ts
  mensagem de commit: "T-309 connect-graphiql-sse: Serve an SSE-aware GraphiQL page"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high >> "$LOG_DIR/seq.log" 2>&1; then
    ...
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-309 connect-graphiql-sse: Serve an SSE-aware GraphiQL page (auto-commit do plano)'
    fi
    marcar_concluidas T-309
    verde "✔ T-309 concluída"
    return 0
  fi
  vermelho "✘ T-309 falhou (log: $LOG_DIR/seq.log)"
  amarelo "  reexecute só ela: bash .spec/features/connect-graphiql-sse/executar-tarefas.sh --seq T-309"
  FALHAS="$FALHAS T-309"
  return 1
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/connect-graphiql-sse/executar-tarefas.sh --gate"
      exit 0
    fi
...
executar_tudo() {
  ...
  executar_seq_T_309 || true
  encerrar tudo
}

listar() {
  echo "execução: $RUN_ID (feature $FEATURE, branch $BASE_BRANCH)"
  echo "  seq       T-309 (sequencial)"
  echo
...
case "$MODO" in
  ...
  seq)
    case "$ALVO" in
      T-309) evento --tipo inicio --escopo "seq:T-309"; iniciar_resumos; executar_seq_T_309 || true; encerrar "seq:T-309" ;;
