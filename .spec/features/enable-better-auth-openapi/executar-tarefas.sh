#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano enable-better-auth-openapi` em 2026-09-11 15:01
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo enable-better-auth-openapi
set -u
set -o pipefail

RUN_ID='desafio-dev-backend-senior-enable-better-auth-openapi-mtx33xk6'
FEATURE='enable-better-auth-openapi'
BASE_BRANCH='spec/enable-better-auth-openapi'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/enable-better-auth-openapi/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-enable-better-auth-openapi-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-enable-better-auth-openapi"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/enable-better-auth-openapi/executar-tarefas.sh --faixa $1"
...
# ── sequencial T-306 (ordem do tasks.md) ──
executar_seq_T_306() {
  info 'sequencial T-306 — Enable Better Auth OpenAPI reference'
  if rodar_tarefa seq 'T-306' 'Você executa UMA tarefa da feature "enable-better-auth-openapi" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/enable-better-auth-openapi/spec.md, .spec/features/enable-better-auth-openapi/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-306 — "Enable Better Auth OpenAPI reference"
  critérios/refs: AC-355 (Referência interativa disponível)
  arquivos permitidos (e seus testes): libs/identity/nest/src/better-auth/better-auth.factory.spec.ts, libs/identity/nest/src/better-auth/better-auth.factory.ts
  mensagem de commit: "T-306 enable-better-auth-openapi: Enable Better Auth OpenAPI reference"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high >> "$LOG_DIR/seq.log" 2>&1; then
    ...
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-306 enable-better-auth-openapi: Enable Better Auth OpenAPI reference (auto-commit do plano)'
    fi
    marcar_concluidas T-306
    verde "✔ T-306 concluída"
    return 0
  fi
  vermelho "✘ T-306 falhou (log: $LOG_DIR/seq.log)"
  amarelo "  reexecute só ela: bash .spec/features/enable-better-auth-openapi/executar-tarefas.sh --seq T-306"
  FALHAS="$FALHAS T-306"
  return 1
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/enable-better-auth-openapi/executar-tarefas.sh --gate"
      exit 0
    fi
...
executar_tudo() {
  ...
  executar_seq_T_306 || true
  encerrar tudo
}

listar() {
  echo "execução: $RUN_ID (feature $FEATURE, branch $BASE_BRANCH)"
  echo "  seq       T-306 (sequencial)"
  echo
...
case "$MODO" in
  ...
  seq)
    case "$ALVO" in
      T-306) evento --tipo inicio --escopo "seq:T-306"; iniciar_resumos; executar_seq_T_306 || true; encerrar "seq:T-306" ;;
