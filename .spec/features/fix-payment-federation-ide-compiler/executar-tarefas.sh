#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano fix-payment-federation-ide-compiler` em 2026-09-10 13:18
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo fix-payment-federation-ide-compiler
set -u
set -o pipefail

RUN_ID='desafio-dev-backend-senior-fix-payment-federation-ide-compiler-mtvjz54w'
FEATURE='fix-payment-federation-ide-compiler'
BASE_BRANCH='spec/fix-payment-federation-ide-compiler'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/fix-payment-federation-ide-compiler/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-fix-payment-federation-ide-compiler-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-fix-payment-federation-ide-compiler"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/fix-payment-federation-ide-compiler/executar-tarefas.sh --faixa $1"
...
# ── sequencial T-258 (ordem do tasks.md) ──
executar_seq_T_258() {
  info 'sequencial T-258 — Align Payment Federation builds with Java 26'
  if rodar_tarefa seq 'T-258' 'Você executa UMA tarefa da feature "fix-payment-federation-ide-compiler" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/fix-payment-federation-ide-compiler/spec.md, .spec/features/fix-payment-federation-ide-compiler/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-258 — "Align Payment Federation builds with Java 26"
  critérios/refs: AC-294 (The module provides a pinned Gradle Wrapper), AC-295 (Every Payment Federation build surface uses Java 26)
  arquivos permitidos (e seus testes): apps/payment-federation/build.gradle.kts, apps/payment-federation/Dockerfile, apps/payment-federation/project.json, apps/payment-federation/gradlew, apps/payment-federation/gradlew.bat, apps/payment-federation/gradle/wrapper/gradle-wrapper.jar, apps/payment-federation/gradle/wrapper/gradle-wrapper.properties, apps/payment-federation/src/test/java/dev/desafio/transaction/architecture/AxonBaselineContractTest.java, apps/e2e/project.json, test/payment-federation-gradle-wrapper.test.mjs, test/milestone-7-coverage.test.mjs, test/milestone-8-quality-gate.test.mjs
  mensagem de commit: "T-258 fix-payment-federation-ide-compiler: Align Payment Federation builds with Java 26"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' medium >> "$LOG_DIR/seq.log" 2>&1; then
    ...
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-258 fix-payment-federation-ide-compiler: Align Payment Federation builds with Java 26 (auto-commit do plano)'
    fi
    marcar_concluidas T-258
    verde "✔ T-258 concluída"
    return 0
  fi
  vermelho "✘ T-258 falhou (log: $LOG_DIR/seq.log)"
  amarelo "  reexecute só ela: bash .spec/features/fix-payment-federation-ide-compiler/executar-tarefas.sh --seq T-258"
  FALHAS="$FALHAS T-258"
  return 1
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/fix-payment-federation-ide-compiler/executar-tarefas.sh --gate"
      exit 0
    fi
...
executar_tudo() {
  ...
  executar_seq_T_258 || true
  encerrar tudo
}

listar() {
  echo "execução: $RUN_ID (feature $FEATURE, branch $BASE_BRANCH)"
  echo "  seq       T-258 (sequencial)"
  echo
...
case "$MODO" in
  ...
  seq)
    case "$ALVO" in
      T-258) evento --tipo inicio --escopo "seq:T-258"; iniciar_resumos; executar_seq_T_258 || true; encerrar "seq:T-258" ;;
