#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano graphql-relay-dataloader-closure` em 2026-09-04 18:09
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo graphql-relay-dataloader-closure
set -u
set -o pipefail

RUN_ID='desafio-dev-backend-senior-graphql-relay-dataloader-closure-mtn9qgak'
FEATURE='graphql-relay-dataloader-closure'
BASE_BRANCH='spec/graphql-relay-dataloader-closure'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/graphql-relay-dataloader-closure/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-graphql-relay-dataloader-closure-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-graphql-relay-dataloader-closure"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/graphql-relay-dataloader-closure/executar-tarefas.sh --faixa $1"
...
# ── sequencial T-166 (ordem do tasks.md) ──
executar_seq_T_166() {
  info 'sequencial T-166 — Preserve commercial Relay edges through federation'
  if rodar_tarefa seq 'T-166' 'Você executa UMA tarefa da feature "graphql-relay-dataloader-closure" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/graphql-relay-dataloader-closure/spec.md, .spec/features/graphql-relay-dataloader-closure/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-166 — "Preserve commercial Relay edges through federation"
  critérios/refs: AC-200 (Commercial lists preserve Relay edges through federation)
  arquivos permitidos (e seus testes): libs/contracts/graphql/wordpress/schema.graphql, apps/apollo-mcp/schema.graphql, apps/e2e/src/journey.ts, apps/wordpress-integration/scripts/probe.mjs, test/graphql-relay-dataloader-closure.test.mjs
  mensagem de commit: "T-166 graphql-relay-dataloader-closure: Preserve commercial Relay edges through federation"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high >> "$LOG_DIR/seq.log" 2>&1; then
    ...
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-166 graphql-relay-dataloader-closure: Preserve commercial Relay edges through federation (auto-commit do plano)'
    fi
    marcar_concluidas T-166
    verde "✔ T-166 concluída"
    return 0
  fi
  vermelho "✘ T-166 falhou (log: $LOG_DIR/seq.log)"
  amarelo "  reexecute só ela: bash .spec/features/graphql-relay-dataloader-closure/executar-tarefas.sh --seq T-166"
  FALHAS="$FALHAS T-166"
  return 1
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/graphql-relay-dataloader-closure/executar-tarefas.sh --gate"
      exit 0
    fi
...
executar_tudo() {
  ...
  executar_seq_T_166 || true
  encerrar tudo
}

listar() {
  echo "execução: $RUN_ID (feature $FEATURE, branch $BASE_BRANCH)"
  echo "  seq       T-166 (sequencial)"
  echo
...
case "$MODO" in
  ...
  seq)
    case "$ALVO" in
      T-166) evento --tipo inicio --escopo "seq:T-166"; iniciar_resumos; executar_seq_T_166 || true; encerrar "seq:T-166" ;;
