#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano harden-oauth-resource-service` em 2026-09-04 23:44
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo harden-oauth-resource-service
set -u
set -o pipefail

RUN_ID='desafio-dev-backend-senior-harden-oauth-resource-service-mtnlphrn'
FEATURE='harden-oauth-resource-service'
BASE_BRANCH='spec/harden-oauth-resource-service'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/harden-oauth-resource-service/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-harden-oauth-resource-service-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-harden-oauth-resource-service"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/harden-oauth-resource-service/executar-tarefas.sh --faixa $1"
...
# ── sequencial T-175 (ordem do tasks.md) ──
executar_seq_T_175() {
  info 'sequencial T-175 — Organize the OAuth resource feature by responsibility'
  if rodar_tarefa seq 'T-175' 'Você executa UMA tarefa da feature "harden-oauth-resource-service" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/harden-oauth-resource-service/spec.md, .spec/features/harden-oauth-resource-service/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-175 — "Organize the OAuth resource feature by responsibility"
  critérios/refs: AC-215 (Critical verifier coverage meets the project standard)
  arquivos permitidos (e seus testes): libs/platform/nest/src/auth/oauth-request.adapter.ts, libs/platform/nest/src/auth/oauth-resource.errors.ts, libs/platform/nest/src/auth/oauth-resource.guard.spec.ts, libs/platform/nest/src/auth/oauth-resource.guard.ts, libs/platform/nest/src/auth/oauth-resource.module.ts, libs/platform/nest/src/auth/oauth-resource.service.integration.spec.ts, libs/platform/nest/src/auth/oauth-resource.service.spec.ts, libs/platform/nest/src/auth/oauth-resource.service.ts, libs/platform/nest/src/auth/oauth-resource.tokens.ts, libs/platform/nest/src/auth/oauth-resource.types.ts, libs/platform/nest/src/oauth-resource/graphql/oauth-resource.guard.spec.ts, libs/platform/nest/src/oauth-resource/graphql/oauth-resource.guard.ts, libs/platform/nest/src/oauth-resource/graphql/oauth-subject.decorator.ts, libs/platform/nest/src/oauth-resource/graphql/require-scopes.decorator.ts, libs/platform/nest/src/oauth-resource/verification/oauth-request.adapter.ts, libs/platform/nest/src/oauth-resource/verification/oauth-resource.errors.ts, libs/platform/nest/src/oauth-resource/verification/oauth-resource.service.integration.spec.ts, libs/platform/nest/src/oauth-resource/verification/oauth-resource.service.spec.ts, libs/platform/nest/src/oauth-resource/verification/oauth-resource.service.ts, libs/platform/nest/src/oauth-resource/oauth-resource.module.ts, libs/platform/nest/src/oauth-resource/oauth-resource.tokens.ts, libs/platform/nest/src/oauth-resource/oauth-resource.types.ts, libs/platform/nest/src/index.ts, test/harden-oauth-resource-service.test.mjs, test/oauth-resource-server-auth.spec.test.mjs, .spec/features/harden-oauth-resource-service/evidence/tdd.md
  mensagem de commit: "T-175 harden-oauth-resource-service: Organize the OAuth resource feature by responsibility"

Regras inegociáveis:
- Todo critério de aceite referenciado vira teste com @spec:AC-xxx no título.
- NUNCA enfraqueça, pule (skip/todo) ou apague um teste para passar — teste pulado não é prova e o audit acusa.
- Rode os testes localmente com `find test -maxdepth 1 -name '\''*.test.mjs'\'' -print0 | xargs -0 env NODE_ENV=test TSX_TSCONFIG_PATH=$PWD/tsconfig.base.json node --import tsx --test --test-reporter=tap` até passarem.
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high >> "$LOG_DIR/seq.log" 2>&1; then
    ...
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-175 harden-oauth-resource-service: Organize the OAuth resource feature by responsibility (auto-commit do plano)'
    fi
    marcar_concluidas T-175
    verde "✔ T-175 concluída"
    return 0
  fi
  vermelho "✘ T-175 falhou (log: $LOG_DIR/seq.log)"
  amarelo "  reexecute só ela: bash .spec/features/harden-oauth-resource-service/executar-tarefas.sh --seq T-175"
  FALHAS="$FALHAS T-175"
  return 1
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/harden-oauth-resource-service/executar-tarefas.sh --gate"
      exit 0
    fi
...
executar_tudo() {
  ...
  executar_seq_T_175 || true
  encerrar tudo
}

listar() {
  echo "execução: $RUN_ID (feature $FEATURE, branch $BASE_BRANCH)"
  echo "  seq       T-175 (sequencial)"
  echo
...
case "$MODO" in
  ...
  seq)
    case "$ALVO" in
      T-175) evento --tipo inicio --escopo "seq:T-175"; iniciar_resumos; executar_seq_T_175 || true; encerrar "seq:T-175" ;;
