#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano harden-oauth-runtime-configuration` em 2026-09-10 18:45
...
RUN_ID='desafio-dev-backend-senior-harden-oauth-runtime-config-harden-oauth-runtime-configuration-mtvvof25'
...
preparar_ambiente() {
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-harden-oauth-runtime-config-harden-oauth-runtime-configuration-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-harden-oauth-runtime-config-harden-oauth-runtime-configuration"
...
executar_seq_T_269() {
  info 'sequencial T-269 — Harden OAuth runtime configuration and Gateway DPoP replay protection'
...
T-269 — "Harden OAuth runtime configuration and Gateway DPoP replay protection"
  critérios/refs: AC-308 (Resolve compatible issuer configuration after Nest config loading), AC-309 (Reserve DPoP proof identifiers atomically across instances), AC-310 (Deploy canonical origin and shared replay storage)
  arquivos permitidos (e seus testes): package.json, pnpm-lock.yaml, infra/sst.config.ts, libs/identity/nest/src/better-auth/better-auth.factory.ts, libs/identity/nest/src/better-auth/better-auth.factory.spec.ts, libs/platform/nest/src/oauth-resource/oauth-resource.module.ts, libs/platform/nest/src/oauth-resource/oauth-resource.module.spec.ts, libs/platform/nest/src/oauth-resource/oauth-resource.types.ts, libs/platform/nest/src/oauth-resource/verification/oauth-resource.service.ts, libs/platform/nest/src/oauth-resource/verification/oauth-resource.service.spec.ts, libs/gateway/nest/src/auth/gateway-auth.module.ts, libs/gateway/nest/src/auth/gateway-auth.module.spec.ts, libs/gateway/nest/src/infrastructure/auth/dynamo-dpop-replay.store.ts, libs/gateway/nest/src/infrastructure/auth/dynamo-dpop-replay.store.spec.ts, test/harden-oauth-runtime-configuration.test.mjs, docs/evidence/harden-oauth-runtime-configuration/T-269.md
  mensagem de commit: "T-269 harden-oauth-runtime-configuration: Harden OAuth runtime configuration and Gateway DPoP replay protection"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high >> "$LOG_DIR/seq.log" 2>&1; then
    ...
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-269 harden-oauth-runtime-configuration: Harden OAuth runtime configuration and Gateway DPoP replay protection (auto-commit do plano)'
    fi
