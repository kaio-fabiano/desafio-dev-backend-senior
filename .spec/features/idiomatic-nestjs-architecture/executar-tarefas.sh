...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo idiomatic-nestjs-architecture
set -u
set -o pipefail
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/idiomatic-nestjs-architecture/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-idiomatic-nestjs-architecture-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-idiomatic-nestjs-architecture"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/idiomatic-nestjs-architecture/executar-tarefas.sh --faixa $1"
...
T-226 — "Make Identity use cases NestJS-managed providers"
  critérios/refs: AC-272 (Identity use cases are container-managed)
  arquivos permitidos (e seus testes): libs/identity/nest/src/application/use-cases, libs/identity/nest/src/identity.module.ts, libs/identity/nest/src/oauth-issuer, libs/identity/nest/src/registration, libs/identity/nest/src/identity-core.architecture.spec.ts, test/identity-federation-refactor.test.mjs, test/wordpress-registration-graphql.contract.test.mjs
  mensagem de commit: "T-226 idiomatic-nestjs-architecture: Make Identity use cases NestJS-managed providers"
...
T-228 — "Make Gateway flows NestJS-managed providers"
  ...
  arquivos permitidos (e seus testes): libs/gateway/nest/src/application/use-cases, libs/gateway/nest/src/auth, libs/gateway/nest/src/federation, libs/gateway/nest/src/gateway.module.ts, apps/gateway/src/app.module.ts, apps/gateway/src/subscriptions, test/gateway-federation-refactor.test.mjs
...
# ── sequencial T-229 (fora da seleção do usuário) ──
executar_seq_T_229() {
  info 'sequencial T-229 — Audit every NestJS module and close repository evidence'
  if rodar_tarefa seq 'T-229' 'Você executa UMA tarefa da feature "idiomatic-nestjs-architecture" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/idiomatic-nestjs-architecture/spec.md, .spec/features/idiomatic-nestjs-architecture/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-229 — "Audit every NestJS module and close repository evidence"
  critérios/refs: AC-275 (Modules preserve explicit boundaries), AC-276 (One task always starts one fresh Codex session)
  arquivos permitidos (e seus testes): test/idiomatic-nestjs-architecture.test.mjs, docs/architecture/idiomatic-nestjs-module-audit.md, graphify-out
  mensagem de commit: "T-229 idiomatic-nestjs-architecture: Audit every NestJS module and close repository evidence"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high >> "$LOG_DIR/seq.log" 2>&1; then
    ...
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-229 idiomatic-nestjs-architecture: Audit every NestJS module and close repository evidence (auto-commit do plano)'
    fi
    marcar_concluidas T-229
    verde "✔ T-229 concluída"
    return 0
  fi
  vermelho "✘ T-229 falhou (log: $LOG_DIR/seq.log)"
  amarelo "  reexecute só ela: bash .spec/features/idiomatic-nestjs-architecture/executar-tarefas.sh --seq T-229"
  FALHAS="$FALHAS T-229"
  return 1
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/idiomatic-nestjs-architecture/executar-tarefas.sh --gate"
      exit 0
    fi
