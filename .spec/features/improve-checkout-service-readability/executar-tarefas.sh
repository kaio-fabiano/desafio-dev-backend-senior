...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo improve-checkout-service-readability
set -u
set -o pipefail
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/improve-checkout-service-readability/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-improve-checkout-service-readability-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-improve-checkout-service-readability"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/improve-checkout-service-readability/executar-tarefas.sh --faixa $1"
...
# ── sequencial T-274 (ordem do tasks.md) ──
executar_seq_T_274() {
  info 'sequencial T-274 — Separate checkout coordination steps'
  if rodar_tarefa seq 'T-274' 'Você executa UMA tarefa da feature "improve-checkout-service-readability" (fluxo onp-spec, spec-anchored).
...
T-274 — "Separate checkout coordination steps"
  critérios/refs: AC-316 (Refatoração preserva o comportamento do checkout)
  arquivos permitidos (e seus testes): apps/payment-federation/src/main/java/dev/desafio/transaction/transaction/checkout/CheckoutService.java, apps/payment-federation/src/test/java/dev/desafio/transaction/transaction/checkout/CheckoutServiceTest.java
  mensagem de commit: "T-274 improve-checkout-service-readability: Separate checkout coordination steps"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high >> "$LOG_DIR/seq.log" 2>&1; then
    ...
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-274 improve-checkout-service-readability: Separate checkout coordination steps (auto-commit do plano)'
    fi
    marcar_concluidas T-274
    verde "✔ T-274 concluída"
    return 0
  fi
  vermelho "✘ T-274 falhou (log: $LOG_DIR/seq.log)"
  amarelo "  reexecute só ela: bash .spec/features/improve-checkout-service-readability/executar-tarefas.sh --seq T-274"
  FALHAS="$FALHAS T-274"
  return 1
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/improve-checkout-service-readability/executar-tarefas.sh --gate"
      exit 0
    fi
