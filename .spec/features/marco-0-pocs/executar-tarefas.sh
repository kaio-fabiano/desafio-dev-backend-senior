...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo marco-0-pocs
set -u
set -o pipefail
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/marco-0-pocs/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-marco-0-pocs-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-marco-0-pocs"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/marco-0-pocs/executar-tarefas.sh --faixa $1"
...
executar_faixa_1() {
  ...
  preparar_worktree 'faixa-1' 'spec/marco-0-pocs-faixa-1' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-1' --estado executando --tentativa "$(tentativa 'faixa-1')"
  : > "$LOG_DIR/faixa-1.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-1' 'T-004' 'Você executa UMA tarefa da feature "marco-0-pocs" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/marco-0-pocs/spec.md, .spec/features/marco-0-pocs/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-004 — "Validate graphql-sse in the federated gateway"
  critérios/refs: AC-009 (Event crosses gateway and subgraph through SSE), AC-010 (Compatibility failure produces a reproducible decision)
  arquivos permitidos (e seus testes): apps/poc-sse/package.json, apps/poc-sse/tsconfig.json, apps/poc-sse/src/gateway.ts, apps/poc-sse/src/subgraph.ts, apps/poc-sse/src/probe.ts, apps/poc-sse/project.json, test/marco-0-sse.test.mjs, docs/adrs/001-graphql-sse-federado.md
  mensagem de commit: "T-004 marco-0-pocs: Validate graphql-sse in the federated gateway"

Regras inegociáveis:
- Todo critério de aceite referenciado vira teste com @spec:AC-xxx no título.
- NUNCA enfraqueça, pule (skip/todo) ou apague um teste para passar — teste pulado não é prova e o audit acusa.
- Rode os testes localmente com `node test/project-planning-memory.test.mjs; node --test --test-reporter=tap test/marco-0-workspace.test.mjs test/marco-0-pocs.spec.test.js` até passarem.
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high
  ...
  local st=$?
  mesclar_faixa 'faixa-1' 'spec/marco-0-pocs-faixa-1' "$WT" "$st" || return 1
...
executar_faixa_2() {
  ...
  preparar_worktree 'faixa-2' 'spec/marco-0-pocs-faixa-2' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-2' --estado executando --tentativa "$(tentativa 'faixa-2')"
  : > "$LOG_DIR/faixa-2.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-2' 'T-005' 'Você executa UMA tarefa da feature "marco-0-pocs" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/marco-0-pocs/spec.md, .spec/features/marco-0-pocs/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-005 — "Validate the Better Auth token at the gateway and MCP"
  critérios/refs: AC-011 (The same grant serves gateway and MCP), AC-012 (Missing audience is rejected)
  arquivos permitidos (e seus testes): apps/poc-auth/package.json, apps/poc-auth/tsconfig.json, apps/poc-auth/src/auth-server.ts, apps/poc-auth/src/resource-servers.ts, apps/poc-auth/src/probe.ts, apps/poc-auth/project.json, test/marco-0-auth.test.mjs, docs/adrs/002-oauth-multi-resource.md
  mensagem de commit: "T-005 marco-0-pocs: Validate the Better Auth token at the gateway and MCP"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high
  ...
  local st=$?
  mesclar_faixa 'faixa-2' 'spec/marco-0-pocs-faixa-2' "$WT" "$st" || return 1
...
executar_faixa_3() {
  ...
  preparar_worktree 'faixa-3' 'spec/marco-0-pocs-faixa-3' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-3' --estado executando --tentativa "$(tentativa 'faixa-3')"
  : > "$LOG_DIR/faixa-3.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-3' 'T-006' 'Você executa UMA tarefa da feature "marco-0-pocs" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/marco-0-pocs/spec.md, .spec/features/marco-0-pocs/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-006 — "Validate WordPress plugin composition"
  critérios/refs: AC-013 (WordPress composes into the supergraph), AC-014 (Critical Woo capabilities have evidence)
  arquivos permitidos (e seus testes): apps/poc-wordpress/package.json, apps/poc-wordpress/project.json, apps/poc-wordpress/compose.yaml, apps/poc-wordpress/scripts/install-plugins.sh, apps/poc-wordpress/scripts/probe.mjs, apps/poc-wordpress/fixtures/products.json, test/marco-0-wordpress.test.mjs, docs/adrs/003-wordpress-federation.md
  mensagem de commit: "T-006 marco-0-pocs: Validate WordPress plugin composition"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high
  ...
  local st=$?
  mesclar_faixa 'faixa-3' 'spec/marco-0-pocs-faixa-3' "$WT" "$st" || return 1
...
executar_seq_T_007() {
  info 'sequencial T-007 — Pin versions, deadline, and decisions in ADRs'
  if rodar_tarefa seq 'T-007' 'Você executa UMA tarefa da feature "marco-0-pocs" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/marco-0-pocs/spec.md, .spec/features/marco-0-pocs/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-007 — "Pin versions, deadline, and decisions in ADRs"
  critérios/refs: AC-015 (Every proof closes a decision with evidence), AC-016 (Delivery constraints remain explicit)
  arquivos permitidos (e seus testes): docs/adrs/README.md, docs/adrs/004-restricoes-de-entrega.md, docs/prds/08-riscos-e-decisoes-pendentes.md, test/marco-0-decisions.test.mjs
  mensagem de commit: "T-007 marco-0-pocs: Pin versions, deadline, and decisions in ADRs"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-luna' low >> "$LOG_DIR/seq.log" 2>&1; then
    ...
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-007 marco-0-pocs: Pin versions, deadline, and decisions in ADRs (auto-commit do plano)'
    fi
  ...
  amarelo "  reexecute só ela: bash .spec/features/marco-0-pocs/executar-tarefas.sh --seq T-007"
  FALHAS="$FALHAS T-007"
  return 1
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/marco-0-pocs/executar-tarefas.sh --gate"
      exit 0
    fi
...
listar() {
  ...
  echo "  faixa-1  onda 1  T-004"
  echo "  faixa-2  onda 1  T-005"
  echo "  faixa-3  onda 1  T-006"
