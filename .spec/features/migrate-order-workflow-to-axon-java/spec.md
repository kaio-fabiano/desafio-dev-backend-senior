...
| Q-022 | Must legacy Workflow rows be imported, or is an empty-state cutover acceptable? | respondida | Use a clean-start cutover. The executable preflight must abort if any legacy checkout, workflow, inbox, or outbox row exists; no durable row may be silently abandoned. |
...
| Q-025 | Must “WordPress/GraphiQL surface” mean an endpoint hosted by WordPress, or may Gateway GraphiQL be the public surface over the composed WordPress graph? | respondida | Gateway GraphiQL is the approved public surface over the composed graph. WordPress remains the commerce data owner and does not host or proxy subscriptions. |
| Q-026 | What are the retention, lease timeout, abandoned-checkout cleanup, payment-token classification, redaction, and retry rules? | respondida | Checkout leases last 30 seconds; abandoned checkouts are eligible for cleanup after 24 hours; completed operations and inbox/outbox records are retained for 30 days. Payment tokens are sensitive, never enter Integration Events or logs, and are represented outside the provider boundary only by opaque references. Technical retries remain 1, 10, and 60 seconds followed by DLQ. |
