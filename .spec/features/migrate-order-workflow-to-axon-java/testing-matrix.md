...
| AC-292 | `MigrationQualityGateTest.shouldHaveExecutableEvidenceForEveryCriterion()` with `@spec:AC-292` plus repository gates | Meta/acceptance | All relevant runners | T-244–T-253 | Unit, fixture, architecture, integration, contract, E2E, coverage, typecheck, lint, verify, and final CI audit pass with no critical skip. |
...
| WordPress/GraphiQL surface | Open the approved public GraphQL surface, subscribe, drive same/other transaction changes | Acceptance | Compose WordPress/Gateway/Java/RabbitMQ/PostgreSQL | AC-289/291, T-250/252 | Required surface receives only matching immediate updates; unresolved endpoint meaning is a completion blocker. |
...
| Commit failure after approval | Approved -> commit rejection -> approved refund/compensation path | E2E | Same as above | AC-283/284/286, T-251 | Q-024-approved terminal and refund result occurs idempotently; until approval this row is `NOT VERIFIED` and blocks completion. |
