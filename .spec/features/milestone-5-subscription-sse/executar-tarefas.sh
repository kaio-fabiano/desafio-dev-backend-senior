...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo milestone-5-subscription-sse
set -u
set -o pipefail
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/milestone-5-subscription-sse/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-milestone-5-subscription-sse-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-milestone-5-subscription-sse"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/milestone-5-subscription-sse/executar-tarefas.sh --faixa $1"
...
executar_faixa_1() {
  ...
  preparar_worktree 'faixa-1' 'spec/milestone-5-subscription-sse-faixa-1' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-1' --estado executando --tentativa "$(tentativa 'faixa-1')"
  : > "$LOG_DIR/faixa-1.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-1' 'T-037' 'Você executa UMA tarefa da feature "milestone-5-subscription-sse" (fluxo onp-spec, spec-anchored).
...
T-037 — "Publish committed workflow transitions through RabbitMQ"
  critérios/refs: AC-053 (A pre-mutation Card stream reaches completion), AC-054 (A pre-mutation Pix stream returns its stable code)
  arquivos permitidos (e seus testes): apps/commerce-subgraph/src/saga/order-event.consumer.ts, apps/commerce-subgraph/src/subscriptions/order-transition.publisher.ts, apps/commerce-subgraph/src/messaging/rabbitmq.ts, libs/contracts/events/order-workflow-transitioned.v1.schema.json, test/milestone-5-transition-publication.test.mjs
  mensagem de commit: "T-037 milestone-5-subscription-sse: Publish committed workflow transitions through RabbitMQ"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high
  ...
  local st=$?
  mesclar_faixa 'faixa-1' 'spec/milestone-5-subscription-sse-faixa-1' "$WT" "$st" || return 1
...
executar_faixa_2() {
  ...
  preparar_worktree 'faixa-2' 'spec/milestone-5-subscription-sse-faixa-2' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-2' --estado executando --tentativa "$(tentativa 'faixa-2')"
  : > "$LOG_DIR/faixa-2.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-2' 'T-038' 'Você executa UMA tarefa da feature "milestone-5-subscription-sse" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/milestone-5-subscription-sse/spec.md, .spec/features/milestone-5-subscription-sse/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-038 — "Implement the authenticated Commerce SSE subscription source"
  critérios/refs: AC-053 (A pre-mutation Card stream reaches completion), AC-054 (A pre-mutation Pix stream returns its stable code), AC-056 (Operation keys are isolated by authenticated subject), AC-058 (Cancellation, timeout, heartbeat, and backpressure are bounded)
  arquivos permitidos (e seus testes): apps/commerce-subgraph/src/subscriptions/order-events.subscription.ts, apps/commerce-subgraph/src/subscriptions/order-event-broker.ts, apps/commerce-subgraph/src/graphql/commerce.module.ts, apps/commerce-subgraph/src/graphql/commerce.resolver.ts, test/milestone-5-commerce-subscription.test.mjs, test/milestone-5-subscription-lifecycle.test.mjs
  mensagem de commit: "T-038 milestone-5-subscription-sse: Implement the authenticated Commerce SSE subscription source"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high
  ...
  local st=$?
  mesclar_faixa 'faixa-2' 'spec/milestone-5-subscription-sse-faixa-2' "$WT" "$st" || return 1
...
executar_faixa_3() {
  ...
  preparar_worktree 'faixa-3' 'spec/milestone-5-subscription-sse-faixa-3' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-3' --estado executando --tentativa "$(tentativa 'faixa-3')"
  : > "$LOG_DIR/faixa-3.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-3' 'T-039' 'Você executa UMA tarefa da feature "milestone-5-subscription-sse" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/milestone-5-subscription-sse/spec.md, .spec/features/milestone-5-subscription-sse/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-039 — "Add the hybrid GraphQL SSE endpoint to the gateway"
  critérios/refs: AC-055 (Authentication is required before opening the stream), AC-057 (The edge uses GraphQL SSE through both segments), AC-058 (Cancellation, timeout, heartbeat, and backpressure are bounded)
  arquivos permitidos (e seus testes): package.json, pnpm-lock.yaml, apps/gateway/src/app.module.ts, apps/gateway/src/main.ts, apps/gateway/src/subscriptions/sse-handler.ts, apps/gateway/src/subscriptions/commerce-subscription.client.ts, test/milestone-5-gateway-sse.test.mjs
  mensagem de commit: "T-039 milestone-5-subscription-sse: Add the hybrid GraphQL SSE endpoint to the gateway"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high
  ...
  local st=$?
  mesclar_faixa 'faixa-3' 'spec/milestone-5-subscription-sse-faixa-3' "$WT" "$st" || return 1
...
executar_seq_T_040() {
  ...
  if rodar_tarefa seq 'T-040' 'Você executa UMA tarefa da feature "milestone-5-subscription-sse" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/milestone-5-subscription-sse/spec.md, .spec/features/milestone-5-subscription-sse/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-040 — "Assemble the Milestone 5 end-to-end acceptance gate"
  critérios/refs: AC-053 (A pre-mutation Card stream reaches completion), AC-054 (A pre-mutation Pix stream returns its stable code), AC-055 (Authentication is required before opening the stream), AC-056 (Operation keys are isolated by authenticated subject), AC-057 (The edge uses GraphQL SSE through both segments), AC-058 (Cancellation, timeout, heartbeat, and backpressure are bounded), AC-059 (Milestone acceptance covers both terminal journeys)
  arquivos permitidos (e seus testes): apps/poc-harness/project.json, test/milestone-5-subscription-sse.test.mjs, docs/runbooks/milestone-5-subscription-sse.md, onpspec.config.json, .github/workflows/ci.yml
  mensagem de commit: "T-040 milestone-5-subscription-sse: Assemble the Milestone 5 end-to-end acceptance gate"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' medium >> "$LOG_DIR/seq.log" 2>&1; then
    # commit de segurança se o agente esqueceu (rastreabilidade > perfeição)
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-040 milestone-5-subscription-sse: Assemble the Milestone 5 end-to-end acceptance gate (auto-commit do plano)'
    fi
  ...
  amarelo "  reexecute só ela: bash .spec/features/milestone-5-subscription-sse/executar-tarefas.sh --seq T-040"
  FALHAS="$FALHAS T-040"
  return 1
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/milestone-5-subscription-sse/executar-tarefas.sh --gate"
      exit 0
    fi
