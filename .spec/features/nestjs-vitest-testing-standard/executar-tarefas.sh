#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano nestjs-vitest-testing-standard` em 2026-09-04 22:42
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo nestjs-vitest-testing-standard
set -u
set -o pipefail

RUN_ID='desafio-dev-backend-senior-nestjs-vitest-testing-standard-mtnjgx79'
FEATURE='nestjs-vitest-testing-standard'
BASE_BRANCH='spec/nestjs-vitest-testing-standard'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/nestjs-vitest-testing-standard/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-nestjs-vitest-testing-standard-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-nestjs-vitest-testing-standard"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/nestjs-vitest-testing-standard/executar-tarefas.sh --faixa $1"
...
# ── sequencial T-171 (ordem do tasks.md) ──
executar_seq_T_171() {
  info 'sequencial T-171 — Codify the NestJS Vitest and TDD contract'
  if rodar_tarefa seq 'T-171' 'Você executa UMA tarefa da feature "nestjs-vitest-testing-standard" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/nestjs-vitest-testing-standard/spec.md, .spec/features/nestjs-vitest-testing-standard/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-171 — "Codify the NestJS Vitest and TDD contract"
  critérios/refs: AC-208 (The repository defines one official testing contract), AC-209 (Every implementation task follows TDD), AC-210 (Vitest coverage fails below the agreed floor), AC-211 (Every NestJS library owns fast unit-test targets)
  arquivos permitidos (e seus testes): AGENTS.md, .spec/constituicao.md, docs/standards/nestjs-vitest-testing.md, test/nestjs-vitest-testing-standard.test.mjs, .spec/features/nestjs-vitest-testing-standard/evidence/tdd.md
  mensagem de commit: "T-171 nestjs-vitest-testing-standard: Codify the NestJS Vitest and TDD contract"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-luna' low >> "$LOG_DIR/seq.log" 2>&1; then
    ...
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-171 nestjs-vitest-testing-standard: Codify the NestJS Vitest and TDD contract (auto-commit do plano)'
    fi
    marcar_concluidas T-171
    verde "✔ T-171 concluída"
    return 0
  fi
  vermelho "✘ T-171 falhou (log: $LOG_DIR/seq.log)"
  amarelo "  reexecute só ela: bash .spec/features/nestjs-vitest-testing-standard/executar-tarefas.sh --seq T-171"
  FALHAS="$FALHAS T-171"
  return 1
}

# ── sequencial T-172 (ordem do tasks.md) ──
executar_seq_T_172() {
  info 'sequencial T-172 — Install and configure shared Vitest coverage tooling'
  if rodar_tarefa seq 'T-172' 'Você executa UMA tarefa da feature "nestjs-vitest-testing-standard" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/nestjs-vitest-testing-standard/spec.md, .spec/features/nestjs-vitest-testing-standard/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-172 — "Install and configure shared Vitest coverage tooling"
  critérios/refs: AC-210 (Vitest coverage fails below the agreed floor), AC-211 (Every NestJS library owns fast unit-test targets)
  arquivos permitidos (e seus testes): package.json, pnpm-lock.yaml, vitest.config.ts, libs/platform/nest/project.json, libs/platform/nest/tsconfig.spec.json, libs/platform/nest/src/config/environment.factory.spec.ts, test/nest-provider-composition.test.mjs, test/nestjs-vitest-testing-standard.test.mjs, .spec/features/nestjs-vitest-testing-standard/evidence/tdd.md
  mensagem de commit: "T-172 nestjs-vitest-testing-standard: Install and configure shared Vitest coverage tooling"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' medium >> "$LOG_DIR/seq.log" 2>&1; then
    # commit de segurança se o agente esqueceu (rastreabilidade > perfeição)
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-172 nestjs-vitest-testing-standard: Install and configure shared Vitest coverage tooling (auto-commit do plano)'
    fi
    marcar_concluidas T-172
    verde "✔ T-172 concluída"
    return 0
  fi
  vermelho "✘ T-172 falhou (log: $LOG_DIR/seq.log)"
  amarelo "  reexecute só ela: bash .spec/features/nestjs-vitest-testing-standard/executar-tarefas.sh --seq T-172"
  FALHAS="$FALHAS T-172"
  return 1
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/nestjs-vitest-testing-standard/executar-tarefas.sh --gate"
      exit 0
    fi
...
executar_tudo() {
  ...
  executar_seq_T_171 || true
  executar_seq_T_172 || true
  encerrar tudo
}

listar() {
  echo "execução: $RUN_ID (feature $FEATURE, branch $BASE_BRANCH)"
  echo "  seq       T-171 (sequencial)"
  echo "  seq       T-172 (sequencial)"
  echo
...
case "$MODO" in
  ...
  seq)
    case "$ALVO" in
      T-171) evento --tipo inicio --escopo "seq:T-171"; iniciar_resumos; executar_seq_T_171 || true; encerrar "seq:T-171" ;;
      T-172) evento --tipo inicio --escopo "seq:T-172"; iniciar_resumos; executar_seq_T_172 || true; encerrar "seq:T-172" ;;
