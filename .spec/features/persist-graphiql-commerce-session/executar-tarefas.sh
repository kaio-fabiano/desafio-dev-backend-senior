#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano persist-graphiql-commerce-session` em 2026-09-11 17:01
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo persist-graphiql-commerce-session
set -u
set -o pipefail

RUN_ID='desafio-dev-backend-senior-persist-graphiql-commerce-session-mtx7e1kt'
FEATURE='persist-graphiql-commerce-session'
BASE_BRANCH='spec/persist-graphiql-commerce-session'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/persist-graphiql-commerce-session/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-persist-graphiql-commerce-session-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-persist-graphiql-commerce-session"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/persist-graphiql-commerce-session/executar-tarefas.sh --faixa $1"
...
# ── sequencial T-310 (ordem do tasks.md) ──
executar_seq_T_310() {
  info 'sequencial T-310 — Preserve commerce session headers in GraphiQL'
  if rodar_tarefa seq 'T-310' 'Você executa UMA tarefa da feature "persist-graphiql-commerce-session" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/persist-graphiql-commerce-session/spec.md, .spec/features/persist-graphiql-commerce-session/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-310 — "Preserve commerce session headers in GraphiQL"
  critérios/refs: AC-360 (Checkout reutiliza automaticamente a sessão criada pelo carrinho)
  arquivos permitidos (e seus testes): apps/gateway/src/graphiql/gateway-graphiql-page.spec.ts, apps/gateway/src/graphiql/gateway-graphiql-page.ts
  mensagem de commit: "T-310 persist-graphiql-commerce-session: Preserve commerce session headers in GraphiQL"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high >> "$LOG_DIR/seq.log" 2>&1; then
    ...
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-310 persist-graphiql-commerce-session: Preserve commerce session headers in GraphiQL (auto-commit do plano)'
    fi
    marcar_concluidas T-310
    verde "✔ T-310 concluída"
    return 0
  fi
  vermelho "✘ T-310 falhou (log: $LOG_DIR/seq.log)"
  amarelo "  reexecute só ela: bash .spec/features/persist-graphiql-commerce-session/executar-tarefas.sh --seq T-310"
  FALHAS="$FALHAS T-310"
  return 1
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/persist-graphiql-commerce-session/executar-tarefas.sh --gate"
      exit 0
    fi
...
executar_tudo() {
  ...
  executar_seq_T_310 || true
  encerrar tudo
}

listar() {
  echo "execução: $RUN_ID (feature $FEATURE, branch $BASE_BRANCH)"
  echo "  seq       T-310 (sequencial)"
  echo
...
case "$MODO" in
  ...
  seq)
    case "$ALVO" in
      T-310) evento --tipo inicio --escopo "seq:T-310"; iniciar_resumos; executar_seq_T_310 || true; encerrar "seq:T-310" ;;
