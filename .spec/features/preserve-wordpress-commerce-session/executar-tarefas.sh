#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano preserve-wordpress-commerce-session` em 2026-09-11 17:47
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo preserve-wordpress-commerce-session
set -u
set -o pipefail

RUN_ID='desafio-dev-backend-senior-preserve-wordpress-commerce-session-mtx91c8s'
FEATURE='preserve-wordpress-commerce-session'
BASE_BRANCH='spec/preserve-wordpress-commerce-session'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/preserve-wordpress-commerce-session/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-preserve-wordpress-commerce-session-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-preserve-wordpress-commerce-session"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/preserve-wordpress-commerce-session/executar-tarefas.sh --faixa $1"
...
# ── sequencial T-311 (ordem do tasks.md) ──
executar_seq_T_311() {
  info 'sequencial T-311 — Preserve WordPress commerce headers through the gateway'
  if rodar_tarefa seq 'T-311' 'Você executa UMA tarefa da feature "preserve-wordpress-commerce-session" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/preserve-wordpress-commerce-session/spec.md, .spec/features/preserve-wordpress-commerce-session/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-311 — "Preserve WordPress commerce headers through the gateway"
  critérios/refs: AC-361 (WordPress session headers survive the federated round trip), AC-362 (One public bearer authenticates the federated journey)
  arquivos permitidos (e seus testes): libs/gateway/nest/src/federation/gateway-federation.configuration.ts, libs/gateway/nest/src/gateway-path.integration.spec.ts, libs/gateway/nest/src/gateway.module.spec.ts
  mensagem de commit: "T-311 preserve-wordpress-commerce-session: Preserve WordPress commerce headers through the gateway"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high >> "$LOG_DIR/seq.log" 2>&1; then
    ...
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-311 preserve-wordpress-commerce-session: Preserve WordPress commerce headers through the gateway (auto-commit do plano)'
    fi
    marcar_concluidas T-311
    verde "✔ T-311 concluída"
    return 0
  fi
  vermelho "✘ T-311 falhou (log: $LOG_DIR/seq.log)"
  amarelo "  reexecute só ela: bash .spec/features/preserve-wordpress-commerce-session/executar-tarefas.sh --seq T-311"
  FALHAS="$FALHAS T-311"
  return 1
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/preserve-wordpress-commerce-session/executar-tarefas.sh --gate"
      exit 0
    fi
...
executar_tudo() {
  ...
  executar_seq_T_311 || true
  encerrar tudo
}

listar() {
  echo "execução: $RUN_ID (feature $FEATURE, branch $BASE_BRANCH)"
  echo "  seq       T-311 (sequencial)"
  echo
...
case "$MODO" in
  ...
  seq)
    case "$ALVO" in
      T-311) evento --tipo inicio --escopo "seq:T-311"; iniciar_resumos; executar_seq_T_311 || true; encerrar "seq:T-311" ;;
