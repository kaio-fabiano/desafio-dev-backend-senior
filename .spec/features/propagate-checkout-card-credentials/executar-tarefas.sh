#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano propagate-checkout-card-credentials` em 2026-09-10 18:47
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo propagate-checkout-card-credentials
set -u
set -o pipefail

RUN_ID='desafio-dev-backend-senior-auth-payment-fixes-propagate-checkout-card-credentials-mtvvq4di'
FEATURE='propagate-checkout-card-credentials'
BASE_BRANCH='spec/propagate-checkout-card-credentials'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/propagate-checkout-card-credentials/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-auth-payment-fixes-propagate-checkout-card-credentials-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-auth-payment-fixes-propagate-checkout-card-credentials"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/propagate-checkout-card-credentials/executar-tarefas.sh --faixa $1"
...
# ── sequencial T-273 (ordem do tasks.md) ──
executar_seq_T_273() {
  info 'sequencial T-273 — Propagate tokenized Card credentials through the asynchronous workflow'
  if rodar_tarefa seq 'T-273' 'Você executa UMA tarefa da feature "propagate-checkout-card-credentials" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/propagate-checkout-card-credentials/spec.md, .spec/features/propagate-checkout-card-credentials/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-273 — "Propagate tokenized Card credentials through the asynchronous workflow"
  critérios/refs: AC-314 (Carry only tokenized Card credentials to Payment), AC-315 (Preserve idempotency while carrying Card credentials), AC-161 (Card data stays outside the platform)
  ...
  mensagem de commit: "T-273 propagate-checkout-card-credentials: Propagate tokenized Card credentials through the asynchronous workflow"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high >> "$LOG_DIR/seq.log" 2>&1; then
    ...
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-273 propagate-checkout-card-credentials: Propagate tokenized Card credentials through the asynchronous workflow (auto-commit do plano)'
    fi
    marcar_concluidas T-273
    verde "✔ T-273 concluída"
    return 0
  fi
  vermelho "✘ T-273 falhou (log: $LOG_DIR/seq.log)"
  amarelo "  reexecute só ela: bash .spec/features/propagate-checkout-card-credentials/executar-tarefas.sh --seq T-273"
  FALHAS="$FALHAS T-273"
  return 1
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/propagate-checkout-card-credentials/executar-tarefas.sh --gate"
      exit 0
    fi
...
executar_tudo() {
  ...
  executar_seq_T_273 || true
  encerrar tudo
}

listar() {
  echo "execução: $RUN_ID (feature $FEATURE, branch $BASE_BRANCH)"
  echo "  seq       T-273 (sequencial)"
  echo
...
case "$MODO" in
  ...
  seq)
    case "$ALVO" in
      T-273) evento --tipo inicio --escopo "seq:T-273"; iniciar_resumos; executar_seq_T_273 || true; encerrar "seq:T-273" ;;
