#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano propagate-checkout-card-credentials` em 2026-09-10 18:45
...
RUN_ID='desafio-dev-backend-senior-propagate-checkout-card-credentials-propagate-checkout-card-credentials-mtvvnxy5'
...
preparar_ambiente() {
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-propagate-checkout-card-credentials-propagate-checkout-card-credentials-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-propagate-checkout-card-credentials-propagate-checkout-card-credentials"
...
executar_seq_T_270() {
  info 'sequencial T-270 — Propagate the short-lived Card credential through choreography'
  if rodar_tarefa seq 'T-270' 'Você executa UMA tarefa da feature "propagate-checkout-card-credentials" (fluxo onp-spec, spec-anchored).
...
T-270 — "Propagate the short-lived Card credential through choreography"
  critérios/refs: AC-306 (Checkout credentials reach the payment request unchanged)
  ...
  mensagem de commit: "T-270 propagate-checkout-card-credentials: Propagate the short-lived Card credential through choreography"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high >> "$LOG_DIR/seq.log" 2>&1; then
    ...
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-270 propagate-checkout-card-credentials: Propagate the short-lived Card credential through choreography (auto-commit do plano)'
    fi
  ...
  amarelo "  reexecute só ela: bash .spec/features/propagate-checkout-card-credentials/executar-tarefas.sh --seq T-270"
  FALHAS="$FALHAS T-270"
  return 1
}
