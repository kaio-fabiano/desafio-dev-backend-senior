#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano propagate-checkout-payer-email` em 2026-09-12 02:44
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo propagate-checkout-payer-email
set -u
set -o pipefail

RUN_ID='desafio-dev-backend-senior-propagate-checkout-payer-email-mtxs82n7'
FEATURE='propagate-checkout-payer-email'
BASE_BRANCH='spec/propagate-checkout-payer-email'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/propagate-checkout-payer-email/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-propagate-checkout-payer-email-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-propagate-checkout-payer-email"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/propagate-checkout-payer-email/executar-tarefas.sh --faixa $1"
...
# ── sequencial T-312 (ordem do tasks.md) ──
executar_seq_T_312() {
  info 'sequencial T-312 — Carry payer email through Transaction'
  if rodar_tarefa seq 'T-312' 'Você executa UMA tarefa da feature "propagate-checkout-payer-email" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/propagate-checkout-payer-email/spec.md, .spec/features/propagate-checkout-payer-email/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-312 — "Carry payer email through Transaction"
  critérios/refs: AC-363 (Payment receives the checkout payer email)
  ...
  mensagem de commit: "T-312 propagate-checkout-payer-email: Carry payer email through Transaction"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high >> "$LOG_DIR/seq.log" 2>&1; then
    ...
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-312 propagate-checkout-payer-email: Carry payer email through Transaction (auto-commit do plano)'
    fi
    marcar_concluidas T-312
    verde "✔ T-312 concluída"
    return 0
  fi
  vermelho "✘ T-312 falhou (log: $LOG_DIR/seq.log)"
  amarelo "  reexecute só ela: bash .spec/features/propagate-checkout-payer-email/executar-tarefas.sh --seq T-312"
  FALHAS="$FALHAS T-312"
  return 1
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/propagate-checkout-payer-email/executar-tarefas.sh --gate"
      exit 0
    fi
...
executar_tudo() {
  ...
  executar_seq_T_312 || true
  encerrar tudo
}

listar() {
  echo "execução: $RUN_ID (feature $FEATURE, branch $BASE_BRANCH)"
  echo "  seq       T-312 (sequencial)"
  echo
...
case "$MODO" in
  ...
  seq)
    case "$ALVO" in
      T-312) evento --tipo inicio --escopo "seq:T-312"; iniciar_resumos; executar_seq_T_312 || true; encerrar "seq:T-312" ;;
