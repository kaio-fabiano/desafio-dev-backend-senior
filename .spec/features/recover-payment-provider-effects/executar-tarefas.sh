...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo recover-payment-provider-effects
set -u
set -o pipefail
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/recover-payment-provider-effects/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-recover-payment-provider-effects-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-recover-payment-provider-effects"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/recover-payment-provider-effects/executar-tarefas.sh --faixa $1"
...
# ── sequencial T-259 (ordem do tasks.md) ──
executar_seq_T_259() {
  info 'sequencial T-259 — Make payment provider effects retryable and observable'
  if rodar_tarefa seq 'T-259' 'Você executa UMA tarefa da feature "recover-payment-provider-effects" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/recover-payment-provider-effects/spec.md, .spec/features/recover-payment-provider-effects/tasks.md e .spec/constituicao.md.
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high >> "$LOG_DIR/seq.log" 2>&1; then
    ...
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-259 recover-payment-provider-effects: Make payment provider effects retryable and observable (auto-commit do plano)'
    fi
    marcar_concluidas T-259
    verde "✔ T-259 concluída"
    return 0
  fi
  vermelho "✘ T-259 falhou (log: $LOG_DIR/seq.log)"
  amarelo "  reexecute só ela: bash .spec/features/recover-payment-provider-effects/executar-tarefas.sh --seq T-259"
  FALHAS="$FALHAS T-259"
  return 1
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/recover-payment-provider-effects/executar-tarefas.sh --gate"
      exit 0
    fi
...
executar_tudo() {
  ...
  executar_seq_T_259 || true
  encerrar tudo
}

listar() {
  echo "execução: $RUN_ID (feature $FEATURE, branch $BASE_BRANCH)"
  echo "  seq       T-259 (sequencial)"
  echo
...
case "$MODO" in
  ...
  seq)
    case "$ALVO" in
      T-259) evento --tipo inicio --escopo "seq:T-259"; iniciar_resumos; executar_seq_T_259 || true; encerrar "seq:T-259" ;;
