...
## T-290 — Implement the durable race-safe checkout persistence slice [pendente]
...
- Modelo: gpt-5.6-luna
- Esforço: baixo
- Notas: Depends on T-289. This is one atomic persistence slice because the entity cannot remove lease fields while the repository contract and adapter still require them. Read the current repeatable Flyway baseline and `design.md`. Red: against one real PostgreSQL container, assert deterministic operation primary key, real `UNIQUE(subject, operation_key)`, required outcome fields, absence of checkout owner/lease columns and index, two-repository insert race recovery, same pair/different-hash rejection, one-winner conditional `PENDING_WOO -> WOO_CREATION_REQUESTED`, commit visibility from an independent transaction, and outcome/error reload. Green: replace Claim with short-transaction `createOrLoad`; the unique-violation loser reloads by subject/key and validates operation ID, command hash, and Woo reference. Remove owner parameters, persist the monotonic Woo-attempt fact before external work, add record Woo/complete/optional fail/owned find operations, rename `CREATING_WOO` to `WOO_CREATION_REQUESTED`, keep unique Woo identifiers, and remove lease schema/entity/repository code. No select-if-insert guarantee, business lock, second migration tool, or in-memory compatibility shim. Refactor with the focused real PostgreSQL suite green.
