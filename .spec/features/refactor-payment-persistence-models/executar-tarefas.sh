#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano refactor-payment-persistence-models` em 2026-09-10 14:25
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo refactor-payment-persistence-models
set -u
set -o pipefail

RUN_ID='desafio-dev-backend-senior-refactor-payment-orm-refactor-payment-persistence-models-mtvmdgu1'
FEATURE='refactor-payment-persistence-models'
BASE_BRANCH='spec/refactor-payment-persistence-models'
ENGINE='/home/kaiosilva/Projects/desafio-dev-backend-senior/.agents/skills/onp-spec-driven/scripts/onp-spec.mjs'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/refactor-payment-persistence-models/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-refactor-payment-orm-refactor-payment-persistence-models-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-refactor-payment-orm-refactor-payment-persistence-models"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/refactor-payment-persistence-models/executar-tarefas.sh --faixa $1"
...
executar_faixa_1() {
  ...
  preparar_worktree 'faixa-1' 'spec/refactor-payment-persistence-models-faixa-1' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-1' --estado executando --tentativa "$(tentativa 'faixa-1')"
  : > "$LOG_DIR/faixa-1.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-1' 'T-260' 'Você executa UMA tarefa da feature "refactor-payment-persistence-models" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/refactor-payment-persistence-models/spec.md, .spec/features/refactor-payment-persistence-models/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-260 — "Model Inventory persistence with JPA"
  critérios/refs: AC-298 (Persistence modeling respects bounded contexts), AC-299 (Inventory state round-trips without overloaded persistence columns), AC-304 (Flyway remains the schema authority)
  arquivos permitidos (e seus testes): apps/payment-federation/src/main/java/dev/desafio/transaction/inventory, apps/payment-federation/src/main/resources/db/migration/inventory, apps/payment-federation/src/test/java/dev/desafio/transaction/inventory, test/refactor-payment-persistence-inventory.test.mjs
  mensagem de commit: "T-260 refactor-payment-persistence-models: Model Inventory persistence with JPA"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high
  ...
  local st=$?
  mesclar_faixa 'faixa-1' 'spec/refactor-payment-persistence-models-faixa-1' "$WT" "$st" || return 1
...
executar_faixa_2() {
  ...
  preparar_worktree 'faixa-2' 'spec/refactor-payment-persistence-models-faixa-2' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-2' --estado executando --tentativa "$(tentativa 'faixa-2')"
  : > "$LOG_DIR/faixa-2.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-2' 'T-261' 'Você executa UMA tarefa da feature "refactor-payment-persistence-models" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/refactor-payment-persistence-models/spec.md, .spec/features/refactor-payment-persistence-models/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-261 — "Model Payment persistence with JPA"
  critérios/refs: AC-298 (Persistence modeling respects bounded contexts), AC-300 (Payment state preserves financial idempotency through ORM), AC-304 (Flyway remains the schema authority)
  ...
  mensagem de commit: "T-261 refactor-payment-persistence-models: Model Payment persistence with JPA"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high
  ...
  local st=$?
  mesclar_faixa 'faixa-2' 'spec/refactor-payment-persistence-models-faixa-2' "$WT" "$st" || return 1
...
executar_faixa_3() {
  ...
  preparar_worktree 'faixa-3' 'spec/refactor-payment-persistence-models-faixa-3' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-3' --estado executando --tentativa "$(tentativa 'faixa-3')"
  : > "$LOG_DIR/faixa-3.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-3' 'T-262' 'Você executa UMA tarefa da feature "refactor-payment-persistence-models" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/refactor-payment-persistence-models/spec.md, .spec/features/refactor-payment-persistence-models/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-262 — "Model Transaction persistence with JPA"
  critérios/refs: AC-298 (Persistence modeling respects bounded contexts), AC-301 (Transaction state preserves checkout lease and projection behavior), AC-304 (Flyway remains the schema authority)
  ...
  mensagem de commit: "T-262 refactor-payment-persistence-models: Model Transaction persistence with JPA"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high
  ...
  local st=$?
  mesclar_faixa 'faixa-3' 'spec/refactor-payment-persistence-models-faixa-3' "$WT" "$st" || return 1
...
executar_faixa_4() {
  ...
  preparar_worktree 'faixa-4' 'spec/refactor-payment-persistence-models-faixa-4' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-4' --estado executando --tentativa "$(tentativa 'faixa-4')"
  : > "$LOG_DIR/faixa-4.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-4' 'T-263' 'Você executa UMA tarefa da feature "refactor-payment-persistence-models" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/refactor-payment-persistence-models/spec.md, .spec/features/refactor-payment-persistence-models/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-263 — "Model shared AMQP delivery persistence with JPA"
  critérios/refs: AC-298 (Persistence modeling respects bounded contexts), AC-302 (Shared delivery preserves reliable concurrent claims), AC-304 (Flyway remains the schema authority)
  ...
  mensagem de commit: "T-263 refactor-payment-persistence-models: Model shared AMQP delivery persistence with JPA"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high
  ...
  local st=$?
  mesclar_faixa 'faixa-4' 'spec/refactor-payment-persistence-models-faixa-4' "$WT" "$st" || return 1
  marcar_concluidas T-263
  return 0
}

# ── sequencial T-264 (fora da seleção do usuário) ──
executar_seq_T_264() {
  info 'sequencial T-264 — Integrate ORM adapters and enforce the zero-SQL runtime boundary'
  if rodar_tarefa seq 'T-264' 'Você executa UMA tarefa da feature "refactor-payment-persistence-models" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/refactor-payment-persistence-models/spec.md, .spec/features/refactor-payment-persistence-models/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-264 — "Integrate ORM adapters and enforce the zero-SQL runtime boundary"
  critérios/refs: AC-298 (Persistence modeling respects bounded contexts), AC-299 (Inventory state round-trips without overloaded persistence columns), AC-300 (Payment state preserves financial idempotency through ORM), AC-301 (Transaction state preserves checkout lease and projection behavior), AC-302 (Shared delivery preserves reliable concurrent claims), AC-303 (Runtime persistence contains no handwritten SQL), AC-304 (Flyway remains the schema authority)
  arquivos permitidos (e seus testes): apps/payment-federation/build.gradle.kts, apps/payment-federation/src/main/resources/application.yaml, apps/payment-federation/src/main/java/dev/desafio/transaction/configuration, apps/payment-federation/src/main/java/dev/desafio/transaction/migration, apps/payment-federation/src/test/java/dev/desafio/transaction/architecture, apps/payment-federation/src/test/java/dev/desafio/transaction/e2e, apps/payment-federation/src/test/java/dev/desafio/transaction/projection, apps/payment-federation/src/test/java/dev/desafio/transaction/infrastructure/persistence, test/refactor-payment-persistence-models.test.mjs
  mensagem de commit: "T-264 refactor-payment-persistence-models: Integrate ORM adapters and enforce the zero-SQL runtime boundary"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high >> "$LOG_DIR/seq.log" 2>&1; then
    ...
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-264 refactor-payment-persistence-models: Integrate ORM adapters and enforce the zero-SQL runtime boundary (auto-commit do plano)'
    fi
    marcar_concluidas T-264
    verde "✔ T-264 concluída"
    return 0
  fi
  vermelho "✘ T-264 falhou (log: $LOG_DIR/seq.log)"
  amarelo "  reexecute só ela: bash .spec/features/refactor-payment-persistence-models/executar-tarefas.sh --seq T-264"
  FALHAS="$FALHAS T-264"
  return 1
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/refactor-payment-persistence-models/executar-tarefas.sh --gate"
      exit 0
    fi
...
executar_tudo() {
  ...
  executar_seq_T_264 || true
  encerrar tudo
}

listar() {
  echo "execução: $RUN_ID (feature $FEATURE, branch $BASE_BRANCH)"
  echo "  faixa-1  onda 1  T-260"
  echo "  faixa-2  onda 1  T-261"
  echo "  faixa-3  onda 1  T-262"
  echo "  faixa-4  onda 2  T-263"
  echo "  seq       T-264 (sequencial)"
  echo
...
case "$MODO" in
  ...
  seq)
    case "$ALVO" in
      T-264) evento --tipo inicio --escopo "seq:T-264"; iniciar_resumos; executar_seq_T_264 || true; encerrar "seq:T-264" ;;
