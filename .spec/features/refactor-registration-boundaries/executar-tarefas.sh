#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano refactor-registration-boundaries` em 2026-09-05 16:16
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo refactor-registration-boundaries
set -u
set -o pipefail

RUN_ID='desafio-dev-backend-senior-refactor-registration-boundaries-mtol58c8'
FEATURE='refactor-registration-boundaries'
BASE_BRANCH='spec/refactor-registration-boundaries'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/refactor-registration-boundaries/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-refactor-registration-boundaries-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-refactor-registration-boundaries"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/refactor-registration-boundaries/executar-tarefas.sh --faixa $1"
...
# ── sequencial T-201 (ordem do tasks.md) ──
executar_seq_T_201() {
  info 'sequencial T-201 — Clarify OAuth issuer and resource-server ownership'
  if rodar_tarefa seq 'T-201' 'Você executa UMA tarefa da feature "refactor-registration-boundaries" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/refactor-registration-boundaries/spec.md, .spec/features/refactor-registration-boundaries/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-201 — "Clarify OAuth issuer and resource-server ownership"
  critérios/refs: AC-244 (Name OAuth components by their protocol responsibility)
  arquivos permitidos (e seus testes): libs/identity/nest/src/oauth/oauth.module.ts, libs/identity/nest/src/oauth/oauth.module.spec.ts, libs/identity/nest/src/oauth/oauth-client-bootstrap.service.ts, libs/identity/nest/src/oauth/oauth-client-bootstrap.service.spec.ts, libs/identity/nest/src/oauth/oauth-clients.controller.ts, libs/identity/nest/src/oauth/oauth-resources.ts, libs/identity/nest/src/oauth/oauth.error.ts, libs/identity/nest/src/oauth-issuer/oauth-issuer.module.ts, libs/identity/nest/src/oauth-issuer/oauth-issuer.module.spec.ts, libs/identity/nest/src/oauth-issuer/oauth-client-provisioning.service.ts, libs/identity/nest/src/oauth-issuer/oauth-client-provisioning.service.spec.ts, libs/identity/nest/src/oauth-issuer/oauth-clients.controller.ts, libs/identity/nest/src/oauth-issuer/oauth-resources.ts, libs/identity/nest/src/oauth-issuer/oauth.error.ts, libs/identity/nest/src/better-auth/better-auth.factory.ts, libs/identity/nest/src/better-auth/better-auth.factory.spec.ts, libs/identity/nest/src/graphql/identity.resolver.ts, libs/identity/nest/src/identity.module.ts, libs/identity/nest/src/index.ts, apps/identity-subgraph/src/main.ts, test/milestone-6-mcp-oauth.test.mjs, test/oauth-resource-server-auth.spec.test.mjs, test/structural-identity-review.test.mjs, test/milestone-8-identity-gateway.test.mjs, docs/evidence/refactor-registration-boundaries/review.md, .spec/verification/refactor-registration-boundaries.json
  mensagem de commit: "T-201 refactor-registration-boundaries: Clarify OAuth issuer and resource-server ownership"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-luna' low >> "$LOG_DIR/seq.log" 2>&1; then
    ...
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-201 refactor-registration-boundaries: Clarify OAuth issuer and resource-server ownership (auto-commit do plano)'
    fi
    marcar_concluidas T-201
    verde "✔ T-201 concluída"
    return 0
  fi
  vermelho "✘ T-201 falhou (log: $LOG_DIR/seq.log)"
  amarelo "  reexecute só ela: bash .spec/features/refactor-registration-boundaries/executar-tarefas.sh --seq T-201"
  FALHAS="$FALHAS T-201"
  return 1
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/refactor-registration-boundaries/executar-tarefas.sh --gate"
      exit 0
    fi
...
executar_tudo() {
  ...
  executar_seq_T_201 || true
  encerrar tudo
}

listar() {
  echo "execução: $RUN_ID (feature $FEATURE, branch $BASE_BRANCH)"
  echo "  seq       T-201 (sequencial)"
  echo
...
case "$MODO" in
  ...
  seq)
    case "$ALVO" in
      T-201) evento --tipo inicio --escopo "seq:T-201"; iniciar_resumos; executar_seq_T_201 || true; encerrar "seq:T-201" ;;
