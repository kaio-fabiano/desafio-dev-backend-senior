#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano remove-retired-applications` em 2026-08-31 12:46
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo remove-retired-applications
set -u
set -o pipefail

RUN_ID='desafio-dev-backend-senior-remove-retired-applications-mth8g5hc'
FEATURE='remove-retired-applications'
BASE_BRANCH='spec/remove-retired-applications'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/remove-retired-applications/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-remove-retired-applications-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-remove-retired-applications"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/remove-retired-applications/executar-tarefas.sh --faixa $1"
...
executar_seq_T_081() {
  info 'sequencial T-081 — Remove retired runtimes and executable dependencies'
  if rodar_tarefa seq 'T-081' 'Você executa UMA tarefa da feature "remove-retired-applications" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/remove-retired-applications/spec.md, .spec/features/remove-retired-applications/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-081 — "Remove retired runtimes and executable dependencies"
  critérios/refs: AC-104 (Retired application roots no longer exist), AC-105 (Active automation does not depend on retired sources)
  arquivos permitidos (e seus testes): apps/commerce-subgraph, apps/stock-worker, apps/e2e/project.json, apps/wordpress-integration/scripts/probe-checkout.mjs, compose.yaml, infra/sst.config.ts, test
  mensagem de commit: "T-081 remove-retired-applications: Remove retired runtimes and executable dependencies"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high >> "$LOG_DIR/seq.log" 2>&1; then
    ...
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-081 remove-retired-applications: Remove retired runtimes and executable dependencies (auto-commit do plano)'
    fi
  ...
  amarelo "  reexecute só ela: bash .spec/features/remove-retired-applications/executar-tarefas.sh --seq T-081"
  FALHAS="$FALHAS T-081"
  return 1
}
...
executar_seq_T_082() {
  info 'sequencial T-082 — Verify and document the reduced project graph'
  if rodar_tarefa seq 'T-082' 'Você executa UMA tarefa da feature "remove-retired-applications" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/remove-retired-applications/spec.md, .spec/features/remove-retired-applications/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-082 — "Verify and document the reduced project graph"
  critérios/refs: AC-104 (Retired application roots no longer exist), AC-105 (Active automation does not depend on retired sources), AC-106 (Supported project gates remain green)
  arquivos permitidos (e seus testes): README.md, docs/knowledge/Mapa do Projeto.md, docs/runbooks, docs/prds, test/retired-applications-removal.test.mjs, .spec/verification/remove-retired-applications.json
  mensagem de commit: "T-082 remove-retired-applications: Verify and document the reduced project graph"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-luna' low >> "$LOG_DIR/seq.log" 2>&1; then
    # commit de segurança se o agente esqueceu (rastreabilidade > perfeição)
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-082 remove-retired-applications: Verify and document the reduced project graph (auto-commit do plano)'
    fi
  ...
  amarelo "  reexecute só ela: bash .spec/features/remove-retired-applications/executar-tarefas.sh --seq T-082"
  FALHAS="$FALHAS T-082"
  return 1
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/remove-retired-applications/executar-tarefas.sh --gate"
      exit 0
    fi
