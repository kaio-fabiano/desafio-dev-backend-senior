#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano remove-wordpress-federation-runtime` em 2026-08-31 23:08
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo remove-wordpress-federation-runtime
set -u
set -o pipefail

RUN_ID='desafio-dev-backend-senior-remove-wordpress-federation-runtime-mthuo64i'
FEATURE='remove-wordpress-federation-runtime'
BASE_BRANCH='spec/remove-wordpress-federation-runtime'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/remove-wordpress-federation-runtime/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-remove-wordpress-federation-runtime-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-remove-wordpress-federation-runtime"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/remove-wordpress-federation-runtime/executar-tarefas.sh --faixa $1"
...
executar_faixa_1() {
  ...
  preparar_worktree 'faixa-1' 'spec/remove-wordpress-federation-runtime-faixa-1' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-1' --estado executando --tentativa "$(tentativa 'faixa-1')"
  : > "$LOG_DIR/faixa-1.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-1' 'T-002' 'Você executa UMA tarefa da feature "remove-wordpress-federation-runtime" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/remove-wordpress-federation-runtime/spec.md, .spec/features/remove-wordpress-federation-runtime/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-002 — "Route integrations directly to native WordPress"
  critérios/refs: AC-117 (Direct plugin subgraph), AC-120 (WordPress capabilities preserved)
  arquivos permitidos (e seus testes): libs/gateway/nest/src/gateway.module.ts, libs/contracts/graphql/supergraph.yaml, compose.yaml, apps/wordpress-integration/compose.yaml, apps/wordpress-integration/scripts/install-plugins.sh, apps/e2e/src/environment.ts
  mensagem de commit: "T-002 remove-wordpress-federation-runtime: Route integrations directly to native WordPress"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high
  ...
  local st=$?
  mesclar_faixa 'faixa-1' 'spec/remove-wordpress-federation-runtime-faixa-1' "$WT" "$st" || return 1
...
executar_faixa_2() {
  ...
  preparar_worktree 'faixa-2' 'spec/remove-wordpress-federation-runtime-faixa-2' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-2' --estado executando --tentativa "$(tentativa 'faixa-2')"
  : > "$LOG_DIR/faixa-2.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-2' 'T-003' 'Você executa UMA tarefa da feature "remove-wordpress-federation-runtime" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/remove-wordpress-federation-runtime/spec.md, .spec/features/remove-wordpress-federation-runtime/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-003 — "Remove the redundant WordPress NestJS runtime"
  critérios/refs: AC-118 (Single subscription owner), AC-119 (Reduced deployable topology)
  arquivos permitidos (e seus testes): apps/wordpress-federation, libs/wordpress/nest, package.json, tsconfig.base.json, test/wordpress-federation-refactor.test.mjs, test/order-subscription-refactor.test.mjs, test/typescript-editor-stability.test.mjs, test/milestone-7-nx-quality.test.mjs, test/milestone-7-containers.test.mjs, test/milestone-7-e2e-contract.test.mjs, test/milestone-8-real-e2e.test.mjs, test/delivery-closure-inventory-saga.test.mjs
  mensagem de commit: "T-003 remove-wordpress-federation-runtime: Remove the redundant WordPress NestJS runtime"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high
  ...
  local st=$?
  mesclar_faixa 'faixa-2' 'spec/remove-wordpress-federation-runtime-faixa-2' "$WT" "$st" || return 1
...
executar_faixa_3() {
  ...
  preparar_worktree 'faixa-3' 'spec/remove-wordpress-federation-runtime-faixa-3' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-3' --estado executando --tentativa "$(tentativa 'faixa-3')"
  : > "$LOG_DIR/faixa-3.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-3' 'T-004' 'Você executa UMA tarefa da feature "remove-wordpress-federation-runtime" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/remove-wordpress-federation-runtime/spec.md, .spec/features/remove-wordpress-federation-runtime/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-004 — "Align architecture documentation with the plugin-first topology"
  critérios/refs: AC-117 (Direct plugin subgraph), AC-118 (Single subscription owner), AC-119 (Reduced deployable topology), AC-120 (WordPress capabilities preserved)
  arquivos permitidos (e seus testes): README.md, docs/adrs/003-wordpress-federation.md, docs/adrs/007-federated-platform-boundaries.md, docs/adrs/README.md, docs/prds/01-arquitetura-e-dominio.md, docs/knowledge/Mapa do Projeto.md, docs/runbooks/local-development.md, docs/runbooks/e2e.md, docs/evidence/federated-platform-refactor/review.md
  mensagem de commit: "T-004 remove-wordpress-federation-runtime: Align architecture documentation with the plugin-first topology"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-luna' low
  ...
  local st=$?
  mesclar_faixa 'faixa-3' 'spec/remove-wordpress-federation-runtime-faixa-3' "$WT" "$st" || return 1
  marcar_concluidas T-004
  return 0
}

# ── sequencial T-001 (fora da seleção do usuário) ──
executar_seq_T_001() {
  info 'sequencial T-001 — Define direct WordPress federation acceptance contracts'
  if rodar_tarefa seq 'T-001' 'Você executa UMA tarefa da feature "remove-wordpress-federation-runtime" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/remove-wordpress-federation-runtime/spec.md, .spec/features/remove-wordpress-federation-runtime/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-001 — "Define direct WordPress federation acceptance contracts"
  critérios/refs: AC-117 (Direct plugin subgraph), AC-118 (Single subscription owner), AC-119 (Reduced deployable topology), AC-120 (WordPress capabilities preserved)
  arquivos permitidos (e seus testes): test/wordpress-direct-federation.test.mjs, test/five-app-topology.test.mjs, test/federated-platform-refactor.test.mjs, test/architecture-boundaries.test.mjs
  mensagem de commit: "T-001 remove-wordpress-federation-runtime: Define direct WordPress federation acceptance contracts"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high >> "$LOG_DIR/seq.log" 2>&1; then
    ...
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-001 remove-wordpress-federation-runtime: Define direct WordPress federation acceptance contracts (auto-commit do plano)'
    fi
  ...
  amarelo "  reexecute só ela: bash .spec/features/remove-wordpress-federation-runtime/executar-tarefas.sh --seq T-001"
  FALHAS="$FALHAS T-001"
  return 1
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/remove-wordpress-federation-runtime/executar-tarefas.sh --gate"
      exit 0
    fi
