#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano resolve-gateway-sse-todos` em 2026-09-01 17:07
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo resolve-gateway-sse-todos
set -u
set -o pipefail

RUN_ID='desafio-dev-backend-senior-resolve-gateway-sse-todos-mtix7pn0'
FEATURE='resolve-gateway-sse-todos'
BASE_BRANCH='spec/resolve-gateway-sse-todos'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/resolve-gateway-sse-todos/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-resolve-gateway-sse-todos-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-resolve-gateway-sse-todos"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/resolve-gateway-sse-todos/executar-tarefas.sh --faixa $1"
...
# ── sequencial T-101 (ordem do tasks.md) ──
executar_seq_T_101() {
  info 'sequencial T-101 — Register and authenticate gateway SSE through NestJS'
  if rodar_tarefa seq 'T-101' 'Você executa UMA tarefa da feature "resolve-gateway-sse-todos" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/resolve-gateway-sse-todos/spec.md, .spec/features/resolve-gateway-sse-todos/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-101 — "Register and authenticate gateway SSE through NestJS"
  critérios/refs: AC-130 (NestJS owns the authenticated SSE route)
  arquivos permitidos (e seus testes): apps/gateway/project.json, apps/gateway/src/main.ts, apps/gateway/src/app.module.ts, apps/gateway/src/subscriptions/sse-handler.ts, apps/gateway/src/subscriptions/sse.middleware.ts, test/remove-wordpress-federation-runtime.spec.test.mjs, test/resolve-gateway-sse-todos.test.mjs
  mensagem de commit: "T-101 resolve-gateway-sse-todos: Register and authenticate gateway SSE through NestJS"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high >> "$LOG_DIR/seq.log" 2>&1; then
    ...
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-101 resolve-gateway-sse-todos: Register and authenticate gateway SSE through NestJS (auto-commit do plano)'
    fi
    marcar_concluidas T-101
    verde "✔ T-101 concluída"
    return 0
  fi
  vermelho "✘ T-101 falhou (log: $LOG_DIR/seq.log)"
  amarelo "  reexecute só ela: bash .spec/features/resolve-gateway-sse-todos/executar-tarefas.sh --seq T-101"
  FALHAS="$FALHAS T-101"
  return 1
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/resolve-gateway-sse-todos/executar-tarefas.sh --gate"
      exit 0
    fi
...
executar_tudo() {
  ...
  executar_seq_T_101 || true
  encerrar tudo
}

listar() {
  echo "execução: $RUN_ID (feature $FEATURE, branch $BASE_BRANCH)"
  echo "  seq       T-101 (sequencial)"
  echo
...
case "$MODO" in
  ...
  seq)
    case "$ALVO" in
      T-101) evento --tipo inicio --escopo "seq:T-101"; iniciar_resumos; executar_seq_T_101 || true; encerrar "seq:T-101" ;;
