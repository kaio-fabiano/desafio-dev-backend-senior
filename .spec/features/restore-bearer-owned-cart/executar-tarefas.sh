#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano restore-bearer-owned-cart` em 2026-09-11 16:02
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo restore-bearer-owned-cart
set -u
set -o pipefail

RUN_ID='desafio-dev-backend-senior-restore-bearer-owned-cart-mtx5af1p'
FEATURE='restore-bearer-owned-cart'
BASE_BRANCH='spec/restore-bearer-owned-cart'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/restore-bearer-owned-cart/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-restore-bearer-owned-cart-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-restore-bearer-owned-cart"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/restore-bearer-owned-cart/executar-tarefas.sh --faixa $1"
...
# ── sequencial T-308 (ordem do tasks.md) ──
executar_seq_T_308() {
  info 'sequencial T-308 — Resolve the native WooCommerce cart from the OAuth subject'
  if rodar_tarefa seq 'T-308' 'Você executa UMA tarefa da feature "restore-bearer-owned-cart" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/restore-bearer-owned-cart/spec.md, .spec/features/restore-bearer-owned-cart/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-308 — "Resolve the native WooCommerce cart from the OAuth subject"
  critérios/refs: AC-357 (The same bearer resolves the same cart), AC-358 (Carts remain isolated by authenticated buyer)
  arquivos permitidos (e seus testes): libs/gateway/nest/src, compose.yaml, test, .spec/features/restore-bearer-owned-cart, .spec/verification/restore-bearer-owned-cart.json
  mensagem de commit: "T-308 restore-bearer-owned-cart: Resolve the native WooCommerce cart from the OAuth subject"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high >> "$LOG_DIR/seq.log" 2>&1; then
    ...
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-308 restore-bearer-owned-cart: Resolve the native WooCommerce cart from the OAuth subject (auto-commit do plano)'
    fi
    marcar_concluidas T-308
    verde "✔ T-308 concluída"
    return 0
  fi
  vermelho "✘ T-308 falhou (log: $LOG_DIR/seq.log)"
  amarelo "  reexecute só ela: bash .spec/features/restore-bearer-owned-cart/executar-tarefas.sh --seq T-308"
  FALHAS="$FALHAS T-308"
  return 1
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/restore-bearer-owned-cart/executar-tarefas.sh --gate"
      exit 0
    fi
...
executar_tudo() {
  ...
  executar_seq_T_308 || true
  encerrar tudo
}

listar() {
  echo "execução: $RUN_ID (feature $FEATURE, branch $BASE_BRANCH)"
  echo "  seq       T-308 (sequencial)"
  echo
...
case "$MODO" in
  ...
  seq)
    case "$ALVO" in
      T-308) evento --tipo inicio --escopo "seq:T-308"; iniciar_resumos; executar_seq_T_308 || true; encerrar "seq:T-308" ;;
