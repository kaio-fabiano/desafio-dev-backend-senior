#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano restore-local-e2e-gateway-runtime` em 2026-09-11 13:12
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo restore-local-e2e-gateway-runtime
set -u
set -o pipefail

RUN_ID='desafio-dev-backend-senior-restore-local-e2e-gateway-runtime-mtwz7ha3'
FEATURE='restore-local-e2e-gateway-runtime'
BASE_BRANCH='spec/restore-local-e2e-gateway-runtime'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/restore-local-e2e-gateway-runtime/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-restore-local-e2e-gateway-runtime-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-restore-local-e2e-gateway-runtime"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/restore-local-e2e-gateway-runtime/executar-tarefas.sh --faixa $1"
...
# ── sequencial T-302 (ordem do tasks.md) ──
executar_seq_T_302() {
  info 'sequencial T-302 — Restore the local Gateway runtime boundary'
  if rodar_tarefa seq 'T-302' 'Você executa UMA tarefa da feature "restore-local-e2e-gateway-runtime" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/restore-local-e2e-gateway-runtime/spec.md, .spec/features/restore-local-e2e-gateway-runtime/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-302 — "Restore the local Gateway runtime boundary"
  critérios/refs: AC-351 (Compose and AWS preserve their distinct replay-storage boundaries)
  arquivos permitidos (e seus testes): compose.yaml, test/milestone-7-e2e-contract.test.mjs
  mensagem de commit: "T-302 restore-local-e2e-gateway-runtime: Restore the local Gateway runtime boundary"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high >> "$LOG_DIR/seq.log" 2>&1; then
    ...
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-302 restore-local-e2e-gateway-runtime: Restore the local Gateway runtime boundary (auto-commit do plano)'
    fi
    marcar_concluidas T-302
    verde "✔ T-302 concluída"
    return 0
  fi
  vermelho "✘ T-302 falhou (log: $LOG_DIR/seq.log)"
  amarelo "  reexecute só ela: bash .spec/features/restore-local-e2e-gateway-runtime/executar-tarefas.sh --seq T-302"
  FALHAS="$FALHAS T-302"
  return 1
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/restore-local-e2e-gateway-runtime/executar-tarefas.sh --gate"
      exit 0
    fi
...
executar_tudo() {
  ...
  executar_seq_T_302 || true
  encerrar tudo
}

listar() {
  echo "execução: $RUN_ID (feature $FEATURE, branch $BASE_BRANCH)"
  echo "  seq       T-302 (sequencial)"
  echo
...
case "$MODO" in
  ...
  seq)
    case "$ALVO" in
      T-302) evento --tipo inicio --escopo "seq:T-302"; iniciar_resumos; executar_seq_T_302 || true; encerrar "seq:T-302" ;;
