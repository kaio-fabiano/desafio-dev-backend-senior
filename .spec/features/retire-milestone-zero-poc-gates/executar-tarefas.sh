#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano retire-milestone-zero-poc-gates` em 2026-09-08 18:48
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo retire-milestone-zero-poc-gates
set -u
set -o pipefail

RUN_ID='desafio-dev-backend-senior-retire-milestone-zero-poc-gates-mtt0vwc6'
FEATURE='retire-milestone-zero-poc-gates'
BASE_BRANCH='spec/retire-milestone-zero-poc-gates'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/retire-milestone-zero-poc-gates/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-retire-milestone-zero-poc-gates-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-retire-milestone-zero-poc-gates"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/retire-milestone-zero-poc-gates/executar-tarefas.sh --faixa $1"
...
# ── sequencial T-230 (ordem do tasks.md) ──
executar_seq_T_230() {
  info 'sequencial T-230 — Retire obsolete PoC gates and preserve current evidence'
  if rodar_tarefa seq 'T-230' 'Você executa UMA tarefa da feature "retire-milestone-zero-poc-gates" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/retire-milestone-zero-poc-gates/spec.md, .spec/features/retire-milestone-zero-poc-gates/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-230 — "Retire obsolete PoC gates and preserve current evidence"
  critérios/refs: AC-277 (Completed Milestone Zero evidence is historical), AC-278 (Default tests avoid the heavy WordPress compatibility probe), AC-279 (Current behavior owns current regression evidence)
  arquivos permitidos (e seus testes): .spec/features/retire-milestone-zero-poc-gates, .spec/features/marco-0-pocs, .spec/verification/marco-0-pocs.json, apps/wordpress-integration/package.json, apps/wordpress-integration/project.json, test/retire-milestone-zero-poc-gates.test.mjs, test/marco-0-workspace.test.mjs, test/marco-0-sse.test.mjs, test/marco-0-auth.test.mjs, test/marco-0-wordpress.test.mjs, test/marco-0-decisions.test.mjs, test/marco-0-pocs.spec.test.js, test/fixtures/auth-probe.ts, test/fixtures/auth-resource-servers.ts, test/fixtures/federated-sse-gateway.ts, test/fixtures/federated-sse-probe.ts, test/fixtures/federated-sse-subgraph.ts, docs/adrs/001-graphql-sse-federado.md, docs/adrs/002-oauth-multi-resource.md, docs/adrs/003-wordpress-federation.md, docs/prds/08-riscos-e-decisoes-pendentes.md, test/reconcile-architecture-decisions.spec.test.mjs, .spec/features/milestone-8-compliance-hardening/tasks.md, .spec/features/refactor-registration-boundaries/tasks.md
  mensagem de commit: "T-230 retire-milestone-zero-poc-gates: Retire obsolete PoC gates and preserve current evidence"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high >> "$LOG_DIR/seq.log" 2>&1; then
    ...
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-230 retire-milestone-zero-poc-gates: Retire obsolete PoC gates and preserve current evidence (auto-commit do plano)'
    fi
    marcar_concluidas T-230
    verde "✔ T-230 concluída"
    return 0
  fi
  vermelho "✘ T-230 falhou (log: $LOG_DIR/seq.log)"
  amarelo "  reexecute só ela: bash .spec/features/retire-milestone-zero-poc-gates/executar-tarefas.sh --seq T-230"
  FALHAS="$FALHAS T-230"
  return 1
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/retire-milestone-zero-poc-gates/executar-tarefas.sh --gate"
      exit 0
    fi
...
executar_tudo() {
  ...
  executar_seq_T_230 || true
  encerrar tudo
}

listar() {
  echo "execução: $RUN_ID (feature $FEATURE, branch $BASE_BRANCH)"
  echo "  seq       T-230 (sequencial)"
  echo
...
case "$MODO" in
  ...
  seq)
    case "$ALVO" in
      T-230) evento --tipo inicio --escopo "seq:T-230"; iniciar_resumos; executar_seq_T_230 || true; encerrar "seq:T-230" ;;
