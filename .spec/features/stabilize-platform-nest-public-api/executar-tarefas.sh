#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano stabilize-platform-nest-public-api` em 2026-09-05 02:10
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo stabilize-platform-nest-public-api
set -u
set -o pipefail

RUN_ID='desafio-dev-backend-senior-stabilize-platform-nest-public-api-mtnqxjs6'
FEATURE='stabilize-platform-nest-public-api'
BASE_BRANCH='spec/stabilize-platform-nest-public-api'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/stabilize-platform-nest-public-api/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-stabilize-platform-nest-public-api-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-stabilize-platform-nest-public-api"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/stabilize-platform-nest-public-api/executar-tarefas.sh --faixa $1"
...
# ── sequencial T-182 (ordem do tasks.md) ──
executar_seq_T_182() {
  info 'sequencial T-182 — Stabilize the platform NestJS public API'
  if rodar_tarefa seq 'T-182' 'Você executa UMA tarefa da feature "stabilize-platform-nest-public-api" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/stabilize-platform-nest-public-api/spec.md, .spec/features/stabilize-platform-nest-public-api/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-182 — "Stabilize the platform NestJS public API"
  critérios/refs: AC-224 (Public entrypoint excludes removed platform abstractions)
  arquivos permitidos (e seus testes): libs/platform/nest/src/index.ts, test/platform-nest-public-api.test.mjs
  mensagem de commit: "T-182 stabilize-platform-nest-public-api: Stabilize the platform NestJS public API"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high >> "$LOG_DIR/seq.log" 2>&1; then
    ...
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-182 stabilize-platform-nest-public-api: Stabilize the platform NestJS public API (auto-commit do plano)'
    fi
    marcar_concluidas T-182
    verde "✔ T-182 concluída"
    return 0
  fi
  vermelho "✘ T-182 falhou (log: $LOG_DIR/seq.log)"
  amarelo "  reexecute só ela: bash .spec/features/stabilize-platform-nest-public-api/executar-tarefas.sh --seq T-182"
  FALHAS="$FALHAS T-182"
  return 1
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/stabilize-platform-nest-public-api/executar-tarefas.sh --gate"
      exit 0
    fi
...
executar_tudo() {
  ...
  executar_seq_T_182 || true
  encerrar tudo
}

listar() {
  echo "execução: $RUN_ID (feature $FEATURE, branch $BASE_BRANCH)"
  echo "  seq       T-182 (sequencial)"
  echo
...
case "$MODO" in
  ...
  seq)
    case "$ALVO" in
      T-182) evento --tipo inicio --escopo "seq:T-182"; iniciar_resumos; executar_seq_T_182 || true; encerrar "seq:T-182" ;;
