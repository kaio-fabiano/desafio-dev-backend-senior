#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano stabilize-typescript-editor` em 2026-08-31 13:58
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo stabilize-typescript-editor
set -u
set -o pipefail

RUN_ID='desafio-dev-backend-senior-stabilize-typescript-editor-mthazlrp'
FEATURE='stabilize-typescript-editor'
BASE_BRANCH='spec/stabilize-typescript-editor'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/stabilize-typescript-editor/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-stabilize-typescript-editor-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-stabilize-typescript-editor"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/stabilize-typescript-editor/executar-tarefas.sh --faixa $1"
...
executar_faixa_1() {
  ...
  preparar_worktree 'faixa-1' 'spec/stabilize-typescript-editor-faixa-1' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-1' --estado executando --tentativa "$(tentativa 'faixa-1')"
  : > "$LOG_DIR/faixa-1.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-1' 'T-083' 'Você executa UMA tarefa da feature "stabilize-typescript-editor" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/stabilize-typescript-editor/spec.md, .spec/features/stabilize-typescript-editor/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-083 — "Add explicit application typecheck projects"
  critérios/refs: AC-107 (Every TypeScript application is typechecked)
  ...
  mensagem de commit: "T-083 stabilize-typescript-editor: Add explicit application typecheck projects"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' medium &&
    rodar_tarefa 'faixa-1' 'T-084' 'Você executa UMA tarefa da feature "stabilize-typescript-editor" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/stabilize-typescript-editor/spec.md, .spec/features/stabilize-typescript-editor/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-084 — "Lock language-specific import behavior"
  critérios/refs: AC-108 (VS Code never organizes TypeScript imports automatically)
  arquivos permitidos (e seus testes): .vscode/settings.json, test/typescript-editor-stability.test.mjs
  mensagem de commit: "T-084 stabilize-typescript-editor: Lock language-specific import behavior"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-luna' low
  ...
  local st=$?
  mesclar_faixa 'faixa-1' 'spec/stabilize-typescript-editor-faixa-1' "$WT" "$st" || return 1
  marcar_concluidas T-083 T-084
  return 0
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/stabilize-typescript-editor/executar-tarefas.sh --gate"
      exit 0
    fi
...
listar() {
  ...
  echo "  faixa-1  onda 1  T-083, T-084"
  echo
