#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano standardize-error-messages` em 2026-09-10 14:53
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo standardize-error-messages
set -u
set -o pipefail

RUN_ID='desafio-dev-backend-senior-standardize-error-messages-mtvndleg'
FEATURE='standardize-error-messages'
BASE_BRANCH='spec/standardize-error-messages'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/standardize-error-messages/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-standardize-error-messages-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-standardize-error-messages"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/standardize-error-messages/executar-tarefas.sh --faixa $1"
...
executar_faixa_1() {
  ...
  preparar_worktree 'faixa-1' 'spec/standardize-error-messages-faixa-1' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-1' --estado executando --tentativa "$(tentativa 'faixa-1')"
  : > "$LOG_DIR/faixa-1.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-1' 'T-261' 'Você executa UMA tarefa da feature "standardize-error-messages" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/standardize-error-messages/spec.md, .spec/features/standardize-error-messages/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-261 — "Standardize Gateway TypeScript errors"
  critérios/refs: AC-298 (TypeScript throw sites use named messages), AC-300 (Custom exceptions provide documented messages), AC-301 (Existing observable error text remains compatible), AC-302 (Future regressions fail the quality gate)
  arquivos permitidos (e seus testes): test/error-message-gateway.test.mjs, apps/gateway/src, libs/gateway/nest/src
  mensagem de commit: "T-261 standardize-error-messages: Standardize Gateway TypeScript errors"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high
  ...
  local st=$?
  mesclar_faixa 'faixa-1' 'spec/standardize-error-messages-faixa-1' "$WT" "$st" || return 1
  marcar_concluidas T-261
  return 0
}
...
executar_faixa_2() {
  ...
  preparar_worktree 'faixa-2' 'spec/standardize-error-messages-faixa-2' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-2' --estado executando --tentativa "$(tentativa 'faixa-2')"
  : > "$LOG_DIR/faixa-2.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-2' 'T-262' 'Você executa UMA tarefa da feature "standardize-error-messages" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/standardize-error-messages/spec.md, .spec/features/standardize-error-messages/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-262 — "Standardize Java Transaction errors"
  critérios/refs: AC-299 (Java throw sites use named messages), AC-300 (Custom exceptions provide documented messages), AC-301 (Existing observable error text remains compatible), AC-302 (Future regressions fail the quality gate)
  arquivos permitidos (e seus testes): test/error-message-java-transaction.test.mjs, apps/payment-federation/src/main/java/dev/desafio/transaction/transaction
  mensagem de commit: "T-262 standardize-error-messages: Standardize Java Transaction errors"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high
  ...
  local st=$?
  mesclar_faixa 'faixa-2' 'spec/standardize-error-messages-faixa-2' "$WT" "$st" || return 1
  marcar_concluidas T-262
  return 0
}
...
executar_faixa_3() {
  ...
  preparar_worktree 'faixa-3' 'spec/standardize-error-messages-faixa-3' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-3' --estado executando --tentativa "$(tentativa 'faixa-3')"
  : > "$LOG_DIR/faixa-3.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-3' 'T-263' 'Você executa UMA tarefa da feature "standardize-error-messages" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/standardize-error-messages/spec.md, .spec/features/standardize-error-messages/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-263 — "Standardize Java Inventory errors"
  critérios/refs: AC-299 (Java throw sites use named messages), AC-300 (Custom exceptions provide documented messages), AC-301 (Existing observable error text remains compatible), AC-302 (Future regressions fail the quality gate)
  arquivos permitidos (e seus testes): test/error-message-java-inventory.test.mjs, apps/payment-federation/src/main/java/dev/desafio/transaction/inventory
  mensagem de commit: "T-263 standardize-error-messages: Standardize Java Inventory errors"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high
  ...
  local st=$?
  mesclar_faixa 'faixa-3' 'spec/standardize-error-messages-faixa-3' "$WT" "$st" || return 1
  marcar_concluidas T-263
  return 0
}
...
executar_faixa_4() {
  ...
  preparar_worktree 'faixa-4' 'spec/standardize-error-messages-faixa-4' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-4' --estado executando --tentativa "$(tentativa 'faixa-4')"
  : > "$LOG_DIR/faixa-4.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-4' 'T-264' 'Você executa UMA tarefa da feature "standardize-error-messages" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/standardize-error-messages/spec.md, .spec/features/standardize-error-messages/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-264 — "Standardize Java Payment errors"
  critérios/refs: AC-299 (Java throw sites use named messages), AC-300 (Custom exceptions provide documented messages), AC-301 (Existing observable error text remains compatible), AC-302 (Future regressions fail the quality gate)
  arquivos permitidos (e seus testes): test/error-message-java-payment.test.mjs, apps/payment-federation/src/main/java/dev/desafio/transaction/payment
  mensagem de commit: "T-264 standardize-error-messages: Standardize Java Payment errors"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high
  ...
  local st=$?
  mesclar_faixa 'faixa-4' 'spec/standardize-error-messages-faixa-4' "$WT" "$st" || return 1
  marcar_concluidas T-264
  return 0
}
...
executar_faixa_5() {
  ...
  preparar_worktree 'faixa-5' 'spec/standardize-error-messages-faixa-5' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-5' --estado executando --tentativa "$(tentativa 'faixa-5')"
  : > "$LOG_DIR/faixa-5.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-5' 'T-265' 'Você executa UMA tarefa da feature "standardize-error-messages" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/standardize-error-messages/spec.md, .spec/features/standardize-error-messages/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-265 — "Standardize Java shared-boundary errors"
  critérios/refs: AC-299 (Java throw sites use named messages), AC-300 (Custom exceptions provide documented messages), AC-301 (Existing observable error text remains compatible), AC-302 (Future regressions fail the quality gate)
  arquivos permitidos (e seus testes): test/error-message-java-shared.test.mjs, apps/payment-federation/src/main/java/dev/desafio/transaction/contracts, apps/payment-federation/src/main/java/dev/desafio/transaction/shared, apps/payment-federation/src/main/java/dev/desafio/transaction/migration
  mensagem de commit: "T-265 standardize-error-messages: Standardize Java shared-boundary errors"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high
  ...
  local st=$?
  mesclar_faixa 'faixa-5' 'spec/standardize-error-messages-faixa-5' "$WT" "$st" || return 1
  marcar_concluidas T-265
  return 0
}
...
executar_faixa_6() {
  ...
  preparar_worktree 'faixa-6' 'spec/standardize-error-messages-faixa-6' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-6' --estado executando --tentativa "$(tentativa 'faixa-6')"
  : > "$LOG_DIR/faixa-6.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-6' 'T-266' 'Você executa UMA tarefa da feature "standardize-error-messages" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/standardize-error-messages/spec.md, .spec/features/standardize-error-messages/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-266 — "Standardize Identity TypeScript errors"
  critérios/refs: AC-298 (TypeScript throw sites use named messages), AC-300 (Custom exceptions provide documented messages), AC-301 (Existing observable error text remains compatible)
  arquivos permitidos (e seus testes): test/error-message-identity.test.mjs, apps/identity-subgraph/src, libs/identity/nest/src
  mensagem de commit: "T-266 standardize-error-messages: Standardize Identity TypeScript errors"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high
  ...
  local st=$?
  mesclar_faixa 'faixa-6' 'spec/standardize-error-messages-faixa-6' "$WT" "$st" || return 1
  marcar_concluidas T-266
  return 0
}
...
executar_faixa_7() {
  ...
  preparar_worktree 'faixa-7' 'spec/standardize-error-messages-faixa-7' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-7' --estado executando --tentativa "$(tentativa 'faixa-7')"
  : > "$LOG_DIR/faixa-7.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-7' 'T-267' 'Você executa UMA tarefa da feature "standardize-error-messages" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/standardize-error-messages/spec.md, .spec/features/standardize-error-messages/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-267 — "Standardize Platform OAuth TypeScript errors"
  critérios/refs: AC-298 (TypeScript throw sites use named messages), AC-300 (Custom exceptions provide documented messages), AC-301 (Existing observable error text remains compatible)
  arquivos permitidos (e seus testes): test/error-message-platform-oauth.test.mjs, libs/platform/nest/src
  mensagem de commit: "T-267 standardize-error-messages: Standardize Platform OAuth TypeScript errors"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high
  ...
  local st=$?
  mesclar_faixa 'faixa-7' 'spec/standardize-error-messages-faixa-7' "$WT" "$st" || return 1
  marcar_concluidas T-267
  return 0
}

# ── sequencial T-260 (fora da seleção do usuário) ──
executar_seq_T_260() {
  info 'sequencial T-260 — Add final executable error-message governance'
  if rodar_tarefa seq 'T-260' 'Você executa UMA tarefa da feature "standardize-error-messages" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/standardize-error-messages/spec.md, .spec/features/standardize-error-messages/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-260 — "Add final executable error-message governance"
  critérios/refs: AC-298 (TypeScript throw sites use named messages), AC-299 (Java throw sites use named messages), AC-300 (Custom exceptions provide documented messages), AC-301 (Existing observable error text remains compatible), AC-302 (Future regressions fail the quality gate)
  arquivos permitidos (e seus testes): tools/error-messages/error-message-policy.mjs, test/error-message-policy.test.mjs, docs/standards/error-message-catalogs.md
  mensagem de commit: "T-260 standardize-error-messages: Add final executable error-message governance"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' medium >> "$LOG_DIR/seq.log" 2>&1; then
    ...
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-260 standardize-error-messages: Add final executable error-message governance (auto-commit do plano)'
    fi
    marcar_concluidas T-260
    verde "✔ T-260 concluída"
    return 0
  fi
  vermelho "✘ T-260 falhou (log: $LOG_DIR/seq.log)"
  amarelo "  reexecute só ela: bash .spec/features/standardize-error-messages/executar-tarefas.sh --seq T-260"
  FALHAS="$FALHAS T-260"
  return 1
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/standardize-error-messages/executar-tarefas.sh --gate"
      exit 0
    fi
...
executar_tudo() {
  ...
  info "onda 1: faixa-1 ∥ faixa-2 ∥ faixa-3 ∥ faixa-4 ∥ faixa-5 ∥ faixa-6 ∥ faixa-7 — janelas limpas em paralelo"
  ...
  executar_seq_T_260 || true
  encerrar tudo
}

listar() {
  echo "execução: $RUN_ID (feature $FEATURE, branch $BASE_BRANCH)"
  echo "  faixa-1  onda 1  T-261"
  echo "  faixa-2  onda 1  T-262"
  echo "  faixa-3  onda 1  T-263"
  echo "  faixa-4  onda 1  T-264"
  echo "  faixa-5  onda 1  T-265"
  echo "  faixa-6  onda 1  T-266"
  echo "  faixa-7  onda 1  T-267"
  echo "  seq       T-260 (sequencial)"
  echo
...
case "$MODO" in
  ...
  seq)
    case "$ALVO" in
      T-260) evento --tipo inicio --escopo "seq:T-260"; iniciar_resumos; executar_seq_T_260 || true; encerrar "seq:T-260" ;;
