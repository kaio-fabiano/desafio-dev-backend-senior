...
1. branch de trabalho `spec/strict-nestjs-ddd-migration` criada do ponto atual (se ainda não existir)
2. as tarefas rodam nela mesma, na ordem — **1 tarefa = 1 commit** (`T-xxx feature: título`), marcada `[concluida]` só com trabalho feito
3. gate final na branch de trabalho: `onp-spec verify strict-nestjs-ddd-migration` + `onp-spec audit --ci` — **exit 0 ou não está pronto**
...
```bash
bash .spec/features/strict-nestjs-ddd-migration/executar-tarefas.sh
```
...
Para gastar menos: `onp-spec plano strict-nestjs-ddd-migration --modelo gpt-5.6-luna --esforco baixo`
(tudo) ou por tarefa `onp-spec tarefa strict-nestjs-ddd-migration T-xxx --modelo <m> --esforco <nível>` — e regenere o plano.
...
```bash
onp-spec resumo strict-nestjs-ddd-migration --tabela   # a tabela de andamento
onp-spec resumo strict-nestjs-ddd-migration            # o resumo em texto
```
