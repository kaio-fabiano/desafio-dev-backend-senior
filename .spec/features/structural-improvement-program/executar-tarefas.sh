#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano structural-improvement-program` em 2026-09-01 01:28
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo structural-improvement-program
set -u
set -o pipefail

RUN_ID='desafio-dev-backend-senior-structural-improvement-program-mthzn7ek'
FEATURE='structural-improvement-program'
BASE_BRANCH='spec/structural-improvement-program'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/structural-improvement-program/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-structural-improvement-program-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/desafio-dev-backend-senior-structural-improvement-program"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/structural-improvement-program/executar-tarefas.sh --faixa $1"
...
executar_faixa_1() {
  ...
  preparar_worktree 'faixa-1' 'spec/structural-improvement-program-faixa-1' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-1' --estado executando --tentativa "$(tentativa 'faixa-1')"
  : > "$LOG_DIR/faixa-1.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-1' 'T-092' 'Você executa UMA tarefa da feature "structural-improvement-program" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/structural-improvement-program/spec.md, .spec/features/structural-improvement-program/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-092 — "Review and improve Gateway boundaries"
  critérios/refs: AC-121 (Gateway remains a thin and secure edge)
  arquivos permitidos (e seus testes): apps/gateway, libs/gateway/nest, test/structural-gateway-review.test.mjs
  mensagem de commit: "T-092 structural-improvement-program: Review and improve Gateway boundaries"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high
  ...
  local st=$?
  mesclar_faixa 'faixa-1' 'spec/structural-improvement-program-faixa-1' "$WT" "$st" || return 1
...
executar_faixa_2() {
  ...
  preparar_worktree 'faixa-2' 'spec/structural-improvement-program-faixa-2' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-2' --estado executando --tentativa "$(tentativa 'faixa-2')"
  : > "$LOG_DIR/faixa-2.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-2' 'T-093' 'Você executa UMA tarefa da feature "structural-improvement-program" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/structural-improvement-program/spec.md, .spec/features/structural-improvement-program/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-093 — "Review and improve Identity boundaries"
  critérios/refs: AC-122 (Identity keeps one authorization authority)
  arquivos permitidos (e seus testes): apps/identity-subgraph, libs/identity/nest, test/structural-identity-review.test.mjs
  mensagem de commit: "T-093 structural-improvement-program: Review and improve Identity boundaries"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high
  ...
  local st=$?
  mesclar_faixa 'faixa-2' 'spec/structural-improvement-program-faixa-2' "$WT" "$st" || return 1
...
executar_faixa_3() {
  ...
  preparar_worktree 'faixa-3' 'spec/structural-improvement-program-faixa-3' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-3' --estado executando --tentativa "$(tentativa 'faixa-3')"
  : > "$LOG_DIR/faixa-3.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-3' 'T-094' 'Você executa UMA tarefa da feature "structural-improvement-program" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/structural-improvement-program/spec.md, .spec/features/structural-improvement-program/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-094 — "Review and improve Commerce workflow boundaries"
  critérios/refs: AC-123 (Commerce keeps deterministic workflow ownership)
  arquivos permitidos (e seus testes): apps/commerce-subgraph, test/structural-commerce-review.test.mjs
  mensagem de commit: "T-094 structural-improvement-program: Review and improve Commerce workflow boundaries"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high
  ...
  local st=$?
  mesclar_faixa 'faixa-3' 'spec/structural-improvement-program-faixa-3' "$WT" "$st" || return 1
...
executar_faixa_4() {
  ...
  preparar_worktree 'faixa-4' 'spec/structural-improvement-program-faixa-4' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-4' --estado executando --tentativa "$(tentativa 'faixa-4')"
  : > "$LOG_DIR/faixa-4.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-4' 'T-095' 'Você executa UMA tarefa da feature "structural-improvement-program" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/structural-improvement-program/spec.md, .spec/features/structural-improvement-program/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-095 — "Review and improve Payment boundaries"
  critérios/refs: AC-124 (Payment keeps reliable isolated adapters)
  arquivos permitidos (e seus testes): apps/payment-processor, test/structural-payment-review.test.mjs
  mensagem de commit: "T-095 structural-improvement-program: Review and improve Payment boundaries"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high
  ...
  local st=$?
  mesclar_faixa 'faixa-4' 'spec/structural-improvement-program-faixa-4' "$WT" "$st" || return 1
...
executar_faixa_5() {
  ...
  preparar_worktree 'faixa-5' 'spec/structural-improvement-program-faixa-5' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-5' --estado executando --tentativa "$(tentativa 'faixa-5')"
  : > "$LOG_DIR/faixa-5.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-5' 'T-096' 'Você executa UMA tarefa da feature "structural-improvement-program" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/structural-improvement-program/spec.md, .spec/features/structural-improvement-program/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-096 — "Review and improve WordPress integration"
  critérios/refs: AC-125 (WordPress bootstrap stays minimal and reproducible)
  arquivos permitidos (e seus testes): apps/wordpress-integration, test/structural-wordpress-review.test.mjs
  mensagem de commit: "T-096 structural-improvement-program: Review and improve WordPress integration"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-5' 'spec/structural-improvement-program-faixa-5' "$WT" "$st" || return 1
...
executar_faixa_6() {
  ...
  preparar_worktree 'faixa-6' 'spec/structural-improvement-program-faixa-6' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-6' --estado executando --tentativa "$(tentativa 'faixa-6')"
  : > "$LOG_DIR/faixa-6.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-6' 'T-097' 'Você executa UMA tarefa da feature "structural-improvement-program" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/structural-improvement-program/spec.md, .spec/features/structural-improvement-program/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-097 — "Review and improve Apollo MCP boundaries"
  critérios/refs: AC-126 (MCP exposes only least-privilege operations)
  arquivos permitidos (e seus testes): apps/apollo-mcp, test/structural-mcp-review.test.mjs
  mensagem de commit: "T-097 structural-improvement-program: Review and improve Apollo MCP boundaries"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-6' 'spec/structural-improvement-program-faixa-6' "$WT" "$st" || return 1
...
executar_faixa_7() {
  ...
  preparar_worktree 'faixa-7' 'spec/structural-improvement-program-faixa-7' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-7' --estado executando --tentativa "$(tentativa 'faixa-7')"
  : > "$LOG_DIR/faixa-7.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-7' 'T-098' 'Você executa UMA tarefa da feature "structural-improvement-program" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/structural-improvement-program/spec.md, .spec/features/structural-improvement-program/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-098 — "Review shared platform and infrastructure"
  critérios/refs: AC-127 (Shared platform and infrastructure stay dependency-safe)
  arquivos permitidos (e seus testes): libs/platform/nest, compose.yaml, nx.json, package.json, tsconfig.base.json, .github, test/structural-platform-review.test.mjs
  mensagem de commit: "T-098 structural-improvement-program: Review shared platform and infrastructure"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-7' 'spec/structural-improvement-program-faixa-7' "$WT" "$st" || return 1
...
executar_faixa_8() {
  ...
  preparar_worktree 'faixa-8' 'spec/structural-improvement-program-faixa-8' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-8' --estado executando --tentativa "$(tentativa 'faixa-8')"
  : > "$LOG_DIR/faixa-8.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-8' 'T-099' 'Você executa UMA tarefa da feature "structural-improvement-program" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/structural-improvement-program/spec.md, .spec/features/structural-improvement-program/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-099 — "Reconcile end-to-end evidence and documentation"
  critérios/refs: AC-128 (End-to-end evidence matches the improved topology)
  arquivos permitidos (e seus testes): apps/e2e, README.md, docs, .spec/features/structural-improvement-program, test/structural-improvement-program.test.mjs
  mensagem de commit: "T-099 structural-improvement-program: Reconcile end-to-end evidence and documentation"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-luna' low
  ...
  local st=$?
  mesclar_faixa 'faixa-8' 'spec/structural-improvement-program-faixa-8' "$WT" "$st" || return 1
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/structural-improvement-program/executar-tarefas.sh --gate"
      exit 0
    fi
...
listar() {
  ...
  echo "  faixa-6  onda 2  T-097"
  echo "  faixa-7  onda 3  T-098"
  echo "  faixa-8  onda 3  T-099"
  echo
