package dev.desafio.payment.graphql;
...
public final class PaymentController {
    ...
    public PaymentView authorizePayment(@Argument("input") AuthorizePayment command, GraphQLContext context) {
        ...
        return authorizePayment.orElseThrow(() -> new IllegalStateException("Payment writes are unavailable"))
            .handle(command);
    }
    ...
    private PaymentView find(String id) {
        return findPayment.orElseThrow(() -> new IllegalStateException("Payment reads are unavailable"))
