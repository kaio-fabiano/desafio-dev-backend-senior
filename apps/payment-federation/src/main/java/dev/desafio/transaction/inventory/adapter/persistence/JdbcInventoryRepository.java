...
public final class JdbcInventoryRepository implements InventoryRepository {
    ...
    private Claim readClaim(
        ...
        try (var statement = connection.prepareStatement("""
            ...
            try (var rows = statement.executeQuery()) {
                ...
                return new Claim(
                    acquired && ownerToken.equals(storedOwner) ? ClaimStatus.ACQUIRED : ClaimStatus.BUSY,
                    request.eventId(),
                    request.operationKey(),
                    acquired ? ownerToken : null,
                    null
                );
            }
        }
    ...
    private Inventory.OutgoingEvent findOutbox(Connection connection, String operationKey) throws SQLException {
        try (var statement = connection.prepareStatement("""
    ...
    private Inventory.OutgoingEvent readEvent(ResultSet row) throws SQLException {
