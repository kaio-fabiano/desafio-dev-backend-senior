create table inventory.inventory_operation (
    ...
    result_event_id uuid references inventory.inventory_outbox (event_id) deferrable initially deferred,
...
create index inventory_operation_expired_claim_index
    on inventory.inventory_operation (lease_until)
    where state = 'CLAIMED';
...
alter table payment.payment_inbox
    alter column effect_id drop not null,
    alter column result_event_id drop not null,
    add column payment_id text references payment.payment_record (payment_id) deferrable initially deferred;
