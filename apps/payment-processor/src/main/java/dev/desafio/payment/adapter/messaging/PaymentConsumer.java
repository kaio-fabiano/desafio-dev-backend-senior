...
public final class PaymentConsumer {
    ...
    public PaymentRepository.ProcessingResult consume(Delivery delivery, Acknowledgement acknowledgement) {
        Objects.requireNonNull(delivery, "delivery");
        Objects.requireNonNull(acknowledgement, "acknowledgement");

        Payment.Command command = switch (delivery.eventType()) {
            case "payment.requested" -> new Payment.PaymentRequested(
                delivery.operationKey(), delivery.paymentId(), delivery.orderId(),
                Payment.Method.valueOf(required(delivery.method(), "method")),
                Objects.requireNonNull(delivery.amount(), "amount"), required(delivery.currency(), "currency")
            );
            case "payment.refund-requested" -> new Payment.RefundRequested(
                delivery.operationKey(), delivery.paymentId(), delivery.orderId(), required(delivery.reason(), "reason")
            );
            default -> throw new IllegalArgumentException("unsupported payment event: " + delivery.eventType());
        };
    ...
    public record Delivery(
        ...
        public static Delivery paymentRequested(
            UUID eventId, String operationKey, String paymentId, String orderId,
            Payment.Method method, BigDecimal amount, String currency
        ) {
            return new Delivery(
                eventId, "payment.requested", operationKey, paymentId, orderId,
                method.name(), amount, currency, null
            );
        }

        public static Delivery refundRequested(
            UUID eventId, String operationKey, String paymentId, String orderId, String reason
        ) {
            return new Delivery(
                eventId, "payment.refund-requested", operationKey, paymentId, orderId,
