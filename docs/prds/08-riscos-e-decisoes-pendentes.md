...
| --- | --- | --- | --- | --- | --- | --- | --- |
| G-001 | P0 | Payment | `PaymentProvider.deterministic()` proves the outbound port and idempotent workflow without moving vendor concerns into the domain. | A selected processor handles Card and Pix in sandbox and production, including signed webhooks, ambiguous outcomes, refunds, provider references, replay safety, and PCI/LGPD controls. | Provider selection, legal/commercial approval, sandbox credentials, webhook ingress, and secret storage. | Provider contract tests, sandbox authorization/refund journeys, invalid-signature rejection, duplicate-webhook replay, timeout reconciliation, and an operational runbook. | Select the provider, change financial invariants, or accept provider-specific operational and compliance constraints. |
