...
test('AC-015: Each proof closes a decision with evidence @spec:AC-015', () => {
  ...
  for (const [adr, evidence] of [
    ...
    [wordpress, ['6.8.2-php8.3-apache', 'ac480974ceb6a1680410f955005e060056f150da', 'node apps/poc-wordpress/scripts/probe.mjs', 'Adopt the indicated plugins']],
  ]) {
...
test('AC-016: Delivery constraints remain explicit @spec:AC-016', () => {
  ...
  assert.match(delivery, /time and timezone are pending/);
  assert.match(prd, /D-008.*decided/);
  assert.match(prd, /D-009.*confirmed date; time pending/);
});
