...
test('AC-063: Tool scopes are enforced @spec:AC-063', async () => {
  await withFixture([REQUIRED_SCOPE], async ({ connect, gateway }) => {
    ...
    try {
      ...
      const mutation = await mcp.callTool({
        name: 'addToCart',
        arguments: { productId: 'product-1', quantity: 1 },
      });
...
test('AC-065: MCP and GraphQL return the same buyer view @spec:AC-065', async () => {
  await withFixture([REQUIRED_SCOPE], async ({ connect, token, gateway }) => {
    ...
    try {
      ...
      assert.deepEqual(
        JSON.parse(result.content[0].text).data.me,
        direct.data.me,
      );
...
test('AC-066: Milestone acceptance is reproducible @spec:AC-066', async () => {
  await withFixture(
    [REQUIRED_SCOPE, CART_WRITE_SCOPE],
    async ({ connect, token, gateway, logs }) => {
      const mcp = await connect();
      try {
        assert.deepEqual(
          (await mcp.listTools()).tools.map(({ name }) => name).sort(),
        ...
        assert.deepEqual(gateway.authorizationHeaders, [
          `Bearer ${token}`,
          `Bearer ${token}`,
          `Bearer ${token}`,
        ]);
        assert.ok(
          logs.every((entry) => !entry.includes(token)),
          'probe logs must redact bearer tokens',
        );
      } finally {
        await mcp.close();
      }
...
async function createFixture(auth, scopesByToken) {
  ...
  const server = createServer(async (request, response) => {
    ...
    const token = authorization.startsWith('Bearer ')
      ...
      : '';
    try {
      await client.verifyAccessTokenRequest(
        new Request(MCP_AUDIENCE, {
    ...
    if (!session && request.method === 'POST' && isInitializeRequest(body)) {
      ...
      const mcp = registeredMcp({
        token,
        scopes: scopesByToken.get(token),
        gateway,
        logs,
      });
  ...
  return {
    ...
    close: () =>
      Promise.all(
        [...sessions.values()].map(({ server: item }) => item.close()),
      )
        .then(() => close(server))
        .then(() => gateway.close()),
  };
...
function registeredMcp({ token, scopes, gateway, logs }) {
  const mcp = new McpServer({
    name: 'apollo-mcp-acceptance-probe',
    version: '1.17.0',
  });
  ...
  for (const [name, scope] of Object.entries(requiredScopes)) {
    mcp.registerTool(name, {}, async (arguments_) => {
      if (!scopes.has(scope))
        return {
          isError: true,
          content: [{ type: 'text', text: `Forbidden: ${scope} is required` }],
        };
      logs.push(`tool=${name}`);
      ...
      response.end(
        JSON.stringify({
          data:
            operationName === 'me'
              ? { me: buyer }
              : { [operationName]: { ok: true } },
        }),
      );
    } catch {
      response
        .writeHead(401)
        .end(JSON.stringify({ errors: [{ message: 'Unauthorized' }] }));
    }
  });
  server.listen(0, '127.0.0.1');
  await once(server, 'listening');
