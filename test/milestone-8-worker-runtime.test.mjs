...
const stockRuntime = await readFile('apps/stock-worker/src/messaging/rabbitmq.runtime.ts', 'utf8');
const stockComposition = await readFile('apps/stock-worker/src/app.module.ts', 'utf8');
...
test('AC-084: real workers persist deduplication and use bounded retry, DLQ, and explicit acknowledgement @spec:AC-084', async () => {
  ...
  const service = new InventoryService({
    async find(id) { return stored.get(id) ?? null; },
    async record(id, result) { if (stored.has(id)) return false; stored.set(id, result); return true; },
  }, {
    async reserve() { effects += 1; },
  }, {
    async publish() { publications += 1; },
  });
  const command = {
    eventId: 'event-84', operationKey: 'checkout-84',
...
test('AC-085: worker builds use pinned workspace or containerized tools and graceful shutdown @spec:AC-085', async () => {
  const [project, dockerfile, gradle, yaml] = await Promise.all([
    ...
    readFile('apps/payment-processor/src/main/resources/application.yaml', 'utf8'),
  ]);
  ...
  assert.match(stockComposition, /await this\.broker\?\.close\(\)/);
...
test('AC-086: worker domain and application code remain independent from runtime frameworks @spec:AC-086', async () => {
  const domain = await Promise.all([
    readFile('apps/payment-processor/src/main/java/dev/desafio/payment/domain/Payment.java', 'utf8'),
    readFile('apps/payment-processor/src/main/java/dev/desafio/payment/application/PaymentHandler.java', 'utf8'),
    readFile('apps/stock-worker/src/inventory/inventory.service.ts', 'utf8'),
  ]).then((files) => files.join('\n'));
  assert.doesNotMatch(domain, /org\.springframework|com\.rabbitmq|amqplib|\bpg\b|@nestjs|fetch\(|node:fs/);
