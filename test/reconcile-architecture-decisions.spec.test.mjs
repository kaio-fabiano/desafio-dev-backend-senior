...
test('AC-185: completed architecture decisions cite executable evidence @spec:AC-185', async () => {
  ...
  assert.match(
    register,
    /D-001[\s\S]*apps\/order-workflow-subgraph\/src\/graphql\/sse\/sse\.integration\.spec\.ts/,
  );
  assert.match(
    register,
    /D-002[\s\S]*test\/oauth-resource-server-auth\.spec\.test\.mjs/,
  );
  assert.match(
    register,
    /D-003[\s\S]*test\/remove-wordpress-federation-runtime\.spec\.test\.mjs/,
  );
  assert.match(
    register,
    /D-011[\s\S]*test\/production-happy-path-hardening\.spec\.test\.js/,
  );
});
...
test('AC-186: decision closure preserves external production gaps @spec:AC-186', async () => {
  ...
  assert.match(
    register,
    /Passing challenge acceptance proves the delivered behavior, not production\s+readiness/,
  );
