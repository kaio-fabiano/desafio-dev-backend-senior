...
test('AC-130: NestJS owns the authenticated SSE route @spec:AC-130', async () => {
  ...
  assert.match(appModule, /ForwardGatewaySubscriptionUseCase/);
  assert.match(appModule, /GatewaySseHandler/);
  assert.match(appModule, /GatewaySseMiddleware/);
  assert.match(middleware, /implements NestMiddleware/);
  assert.match(middleware, /this\.handler\.handle\(request, response\)/);
  assert.doesNotMatch(middleware, /verifyGatewayRequest|issuer|jwksUrl/);
  assert.match(handler, /await this\.authContext\.create\(raw\)/);
  assert.doesNotMatch(handler, /new ForwardGatewaySubscriptionUseCase/);
});
